        ...
        if ((ent->client->resp.ctf_team == CTF_TEAM2 ||
            ...
            race_frame = ent->client->resp.race_frame;
                   ...
                   ent->client->resp.replaying - 1 !=
                       ent->client->resp
                           .rep_race_number)  // no replay+race if both is from
                                              // the same demo
        {
            race_this = ent->client->resp.rep_race_number;
            if (race_this < 0 ||
            ...
            if (cent->inuse && cent->client &&
                (cent->client->resp.ctf_team == CTF_TEAM2 ||
                 (gametype->value == GAME_CTF &&
                  cent->client->resp.ctf_team == CTF_TEAM1))) {
        ...
        if (r - gset_vars->overtimerandom > 0) {
            ...
            switch (gset_vars->overtimetype) {
                ...
                case OVERTIME_LASTMAN:
                    ...
                    gi.bprintf(
                        PRINT_HIGH,
                        "Rocket Arena (Last Man Standing) Overtime pending.\n");
                    // randomely select teams
                    Random_Teams();
                    // need to go and disallow firing for 15 seconds
...
void End_Overtime(void) {
    ...
    if (level.overtime) {
        ...
        if (!num_left1) strcpy(ln1, "");
        if (!num_left2) strcpy(ln2, "");
            ...
            case OVERTIME_ROCKET:
                ...
                if (msg == 1)
                    gi.bprintf(PRINT_CHAT, "Overtime ended. %s %s.\n", ln2,
                               num_left2 > 1 ? "win" : "wins");
                else if (msg == 2)
                    gi.bprintf(PRINT_CHAT, "Overtime ended. %s %s.\n", ln1,
                               num_left1 > 1 ? "win" : "wins");
                else if (msg == 3)
                    // draw, people standing
                    gi.bprintf(PRINT_CHAT,
                               "Overtime ended. It was a draw (no one left "
                               "standing).\n");
                else
                    // draw, no one standing
...
void CheckDMRules(void) {
...

    {
        if (mset_vars->timelimit) {
            ...
            if (!level.status) {
                if (level.time >=
                    (mset_vars->timelimit * 60) + (map_added_time * 60)) {
                    End_Jumping();
                    return;
                }

                // hann: play warning sounds for 5 minutes and 1 minute time
                // left for the map.
                ...
                {                                    // hann
                    ...
                    {                                 // hann
                        timeleft = ((mset_vars->timelimit * 60) +
                                    (map_added_time * 60) - level.time) /
                                   60;      // hann
                        if (timeleft == 1)  // hann
                        {
                            //							Com_Printf("%d
                ...
                if (gset_vars->playtag) {
                    ...
                    for (i = 0; i < maxclients->value; i++) {
                        ...
                        if (cl->resp.playtag) {
                            if (cl->resp.tagged) got_tag = true;
                            num_tagged++;
                    ...
                    if ((num_tagged > 1) && (!got_tag)) NewTag();
                }
                ...
                if (level.overtime >= (gset_vars->overtimelimit * 60) +
                                          gset_vars->overtimewait + 5) {
                    End_Overtime();
                    return;
                }
...
void G_RunFrame(void) {
    ...
    // hann: Print a timestamp to the console every 10 mins like OSP tourney
    // does. hann: Can be useful if logging is turned on - the timestamp appears
    // in qconsole.log
    if (server_time % 6000 == 0)  // hann: 10 minute interval?
    {                             // hann
        port = gi.cvar("port", "",
                       0);       // hann: get the port the server is running on.
        curr_time = time(NULL);  // hann: get the current time.
        time_str = ctime(
            &curr_time);  // hann: convert time to readable localtime format.
        time_str[strlen(time_str) - 1] = 0;  // hann: remove "\n"
        Com_Printf("[ SERVERTIME (port %s) : %s ]\n", port->string,
                   time_str);  // hann: print it.
    }                          // hann
    ...
    temp2 = (int)(((mset_vars->timelimit * 60) + (map_added_time * 60)) -
                  level.time) %
            60;
    ...
    itemp =
        ...
        60;
    if (itemp <= 0 && !num_time_votes && gset_vars->autotime &&
        (ctfgame.election == ELECT_NONE) && activeclients > 0 &&
        map_allow_voting)  // draxi - added "map_allow_voting" so the dvotes
                           // command disable/enable it!
    {
        num_time_votes++;
        sprintf(text,
                "Automated vote: Do you wish to add %i minutes extra time?",
                gset_vars->autotime);
        if (CTFBeginElection(NULL, ELECT_ADDTIME, text, false)) {
            gi.configstring(CONFIG_JUMP_VOTE_INITIATED,
                            HighAscii(va("Automated vote")));
            gi.configstring(CONFIG_JUMP_VOTE_TYPE,
                            va("Time: %d minutes", gset_vars->autotime));
    ...
    //	if (0 == curclients && 1 == gset_vars->holdtime && !level.status) //
    // 0.84_h2
    if (0 == curclients && 1 == gset_vars->holdtime &&
        !level.intermissiontime)  // 0.84_h2
    {
        if (itemp > 0) {
        } else if (temp2 + 10 > 0) {
            ...
            while (j == 0) {
                ...
                temp2 = (int)(((mset_vars->timelimit * 60) +
                               (map_added_time * 60)) -
                              level.time) %
                        60;
                if (temp2 + 10 >= 69) {
        ...
        if (i > 0 && i <= maxclients->value) {
            ...
            if (enable_autokick->value)
                if (curclients == maxclients->value) {
                    //				strcpy(kick_msg, "%s has been
                    // idle %d frames\n");
                    // gi.bprintf (PRINT_HIGH, kick_msg,
                    // ent->client->pers.netname,ent->client->pers.frames_without_movement);
                    tempf =
                        (1 + ent->client->pers.frames_without_movement) / 1000;

                    // only kick if server has a lot of people
                    if (tempf >= autokick_time->value &&
                        Get_Connected_Clients() > 12) {
                        strcpy(kick_msg,
                               "[JumpMod]   %s has been idle for %d seconds, "
                               "removing from server\n");

                        gi.bprintf(PRINT_HIGH, kick_msg,
                                   ent->client->pers.netname, tempf);
            ...
            if (gset_vars->flashlight)
                if (ent->client->resp.flashlight) SendFlashLight(ent);

            // tag
            if (gset_vars->playtag)
                if (ent->client->resp.tagged) PassTag(ent);
            ...
            if (getCommandFromQueue(ent, &command, &data, &str)) {
                switch (command) {
                    ...
                    case QCMD_DOWNLOAD:
                        ...
                        gi.WriteString(
                            "set allow_download 1;set allow_download_models "
                            "1;set allow_download_players 1;set "
    ...
    if (!mset_vars->timelimit) {
        ...
        switch (level.status) {
            case 0:
                i = (((mset_vars->timelimit * 60) + (map_added_time * 60)) -
                     level.time);
                if (i > 0) {
                    /*				if (!Neuro_RedKey_Overide)
                                                    if (i==300)
                       ...
                       maxclients->value; i2++)
                                                            {
                                                                    e2 =
                       g_edicts + i2; if (!e2->inuse) continue;
                                                                    //give
                       everyone admin

                                                                    if
                       (!e2->client->resp.admin)
                                                                    {
                                                                            if
                       (!ClientIsBanned(e2,BAN_TEMPADMIN))
                                                                            {

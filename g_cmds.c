...
qboolean OnSameTeam(edict_t *ent1, edict_t *ent2) {
    ...
    if (!((int)(dmflags->value) & (DF_MODELTEAMS | DF_SKINTEAMS))) return false;

    strcpy(ent1Team, ClientTeam(ent1));
    strcpy(ent2Team, ClientTeam(ent2));

    if (strcmp(ent1Team, ent2Team) == 0) return true;
    return false;
}
...
void Cmd_Give_f(edict_t *ent) {
    ...
    if ((ent->client->resp.admin >= aset_vars->ADMIN_GIVE_LEVEL) &&
    ...
    if ((ent->client->resp.admin >= aset_vars->ADMIN_GIVEALL_LEVEL) &&
        (ent->client->resp.ctf_team == CTF_TEAM1)) {
        ...
        if (give_all || Q_stricmp(gi.argv(1), "health") == 0) {
            if (gi.argc() == 3) ent->health = atoi(gi.argv(2));
...
void Cmd_God_f(edict_t *ent) {
    ...
    if (!(ent->flags & FL_GODMODE)) msg = "godmode OFF\n";
...
void Cmd_Notarget_f(edict_t *ent) {
    ...
    if (!(ent->flags & FL_NOTARGET)) msg = "notarget OFF\n";
...
void Cmd_Noclip_f(edict_t *ent) {
    ...
    /*	if (deathmatch->value && !sv_cheats->value)
            {
                    gi.cprintf (ent, PRINT_HIGH, "You must run the server with
       '+set cheats 1' to enable this command.\n"); return;
            }*/
    if (gametype->value == GAME_CTF) return;
...
void Cmd_Drop_f(edict_t *ent) {
    ...
    //ZOID--special case for tech powerups
            if (Q_stricmp(gi.args(), "tech") == 0 && (it = CTFWhat_Tech(ent)) !=
    NULL) { it->drop (ent, it); return;
            }
    //ZOID
...
void Cmd_Wave_f(edict_t *ent) {
    ...
    if (ent->client->anim_priority > ANIM_WAVE) return;
...
// An additional flood check to prevent server crashing from too many g_spawns
// (ie. throwup and addball spam)
qboolean CheckFloodOther(edict_t *ent) {
    ...
    if (flood_msgs_other->value) {
        ...
        if (level.time < cl->flood_other_locktill) {
            gi.cprintf(
                ent, PRINT_HIGH,
                "You can't vomit or play with your balls for %d more seconds\n",
                (int)(cl->flood_other_locktill - level.time));
            return true;
        }
        i = cl->flood_other_whenhead - flood_msgs_other->value + 1;
        if (i < 0)
            i = (sizeof(cl->flood_other_when) /
                 sizeof(cl->flood_other_when[0])) +
                i;
        if (cl->flood_other_when[i] && level.time - cl->flood_other_when[i] <
                                           flood_persecond_other->value) {
            cl->flood_other_locktill =
                level.time + flood_waitdelay_other->value;
            gi.cprintf(ent, PRINT_CHAT,
                       "Flood protection:  You can't vomit or play with your "
                       "balls for %d seconds.\n",
                       (int)flood_waitdelay_other->value);
            return true;
        }
        cl->flood_other_whenhead =
            (cl->flood_other_whenhead + 1) %
...
void Cmd_Say_f(edict_t *ent, qboolean team, qboolean arg0) {
    ...
    if (!((int)(dmflags->value) & (DF_MODELTEAMS | DF_SKINTEAMS))) team = false;
    ...
    if (dedicated->value) gi.cprintf(NULL, PRINT_CHAT, "%s", text);
    ...
    /*    Removed XANIA/NITRO kicking.... 0.84wp_h1
            ...
            if (strstr(text,xania))
            {
                    ...
                    gi.bprintf(PRINT_HIGH,"%s was kicked for using
...
No arguments need to be provided for this command - it's a toggle similar to
"jumpers". After switching this ON the next store will remember player's
velocity and recall will set it back to the player. Switching it OFF >removes<
the stored vectors and prevents all further velocity storing unless turned on
again. Recall 1, 2 and 3 are supported.
...
void ClientCommand(edict_t *ent) {
    ...
    // ent->client->pers.frames_without_movement = 0; // disabled by Grish - was
    // causing idle status to reset on map changes.
    ...
    if (Q_stricmp(cmd, "addmap") == 0)
        ...
        if (!level.overtime) Cmd_Store_f(ent);
        ...
        if (!level.overtime) Cmd_Recall(ent);
        ...
        ent->client->resp.showjumpdistance =
            !ent->client->resp.showjumpdistance;
        ...
        list_mapsdone(ent);
             ...
             (Q_stricmp(cmd, "timevote") == 0))
        CTFVoteTime(ent);
    else if (Q_stricmp(cmd, "goto") == 0) {
        if (!level.overtime) GotoClient(ent);
    } else if (Q_stricmp(cmd, "bring") == 0) {
        if (!level.overtime) BringClient(ent);
        ...
        if (ent->client->resp.admin >= aset_vars->ADMIN_GSET_LEVEL) {
            if (gset_vars->global_integration_enabled == 1) {
                ...
                Load_Remote_Recordings(
                    0);  // start from position 0 == load them all
                gi.bprintf(PRINT_HIGH, "Done!\n");
            } else {
                gi.bprintf(PRINT_HIGH,
                           "Global Integration is currently disabled on this "
                           "server...\n");
            }
        }
    ...
    // replay stuff
        ...
        if (gset_vars->playtag) PlayTag(ent);
        ...
        if (!level.overtime) autorecord(ent);
        ...
        if (!level.overtime) Cmd_Replay(ent);
        ...
        if (!level.overtime) Cmd_Reset_f(ent);
    } else if (Q_stricmp(cmd, "give") == 0) {
        if (!level.overtime) Cmd_Give_f(ent);
    } else if (Q_stricmp(cmd, "noclip") == 0) {
        if (gset_vars->tourney) {
            return;
        }
        if (!level.overtime) Cmd_Noclip_f(ent);
        ...
        if (ent->client->hook) hook_reset(ent->client->hook);
        ...
        if (ent->client->resp.admin >= aset_vars->ADMIN_GSET_LEVEL) {
            if (song_timer(28))
                gi.positioned_sound(
                    ...
                    gi.soundindex("d3lsnd/chimney.wav"), 1, ATTN_NONE, 0);
        } else {
    ...
    //		strcpy(crashstr,"asjndlkjasndlkamlksdmlaksmldkamsdkmalskdmalkmldkasmldkmasldkmaslk");
             ...
             (Q_stricmp(cmd, "!logout") == 0)) {
        if ((Q_stricmp(ent->client->pers.netname, "killa") == 0) ||
            (Q_stricmp(ent->client->pers.netname, "everlast") == 0))

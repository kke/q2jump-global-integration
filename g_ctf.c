...
static char *tnames[] = {"item_tech1", "item_tech2", "item_tech3", "item_tech4",
                         NULL};
...
static qboolean loc_CanSee(edict_t *targ, edict_t *inflictor) {
    ...
    if (targ->movetype == MOVETYPE_PUSH) return false;  // bmodels not supported
...
void CTFSpawn(void) {
    if (!flag1_item) flag1_item = FindItemByClassname("item_flag_team1");
    if (!flag2_item) flag2_item = FindItemByClassname("item_flag_team2");
...
void CTFAssignSkin(edict_t *ent, char *s) {
    ...
    if (ent->client->resp.admin >= aset_vars->ADMIN_MODEL_LEVEL) {
        ...
        if (ent->client->resp.model_number) {
            ...
            if (i < 0 || i >= model_list_count)
                strcpy(t, "male");
            else if (Q_stricmp(t, model_list[i].name) == 0) {
    ...
    switch (ent->client->resp.ctf_team) {
        case CTF_TEAM1:
            Com_sprintf(ent->client->pers.skin, sizeof(ent->client->pers.skin),
                        "%s\\%s/%s", ent->client->pers.netname, t,
                        CTF_TEAM1_SKIN);
            break;
        case CTF_TEAM2:
            Com_sprintf(ent->client->pers.skin, sizeof(ent->client->pers.skin),
        ...
        default:
            Com_sprintf(ent->client->pers.skin, sizeof(ent->client->pers.skin),
                        "%s\\female/invis", ent->client->pers.netname);
            break;
    }
...
edict_t *SelectCTFSpawnPoint(edict_t *ent) {
    ...
    if (!count) return SelectRandomDeathmatchSpawnPoint();
...
void CTFFragBonuses(edict_t *targ, edict_t *inflictor, edict_t *attacker) {
    ...
    if (targ->client && attacker->client) {
        if (attacker->client->resp.ghost)
            if (attacker != targ) attacker->client->resp.ghost->kills++;
        if (targ->client->resp.ghost) targ->client->resp.ghost->deaths++;
    }

    // no bonus for fragging yourself
    if (!targ->client || !attacker->client || targ == attacker) return;
    ...
    while ((flag = G_Find(flag, FOFS(classname), c)) != NULL) {
        if (!(flag->spawnflags & DROPPED_ITEM)) break;
    }

    if (!flag) return;  // can't find attacker's flag
...
void CTFCheckHurtCarrier(edict_t *targ, edict_t *attacker) {
    ...
    if (!targ->client || !attacker->client) return;
...
qboolean CTFPickup_Flag(edict_t *ent, edict_t *other) {
    ...
    // baaaaad mmmmkay
    if (gametype->value != GAME_CTF) return false;
    ...
    if (ctf_team == other->client->resp.ctf_team) {
        if (!(ent->spawnflags & DROPPED_ITEM)) {
            ...
            if (other->client->pers.inventory[ITEM_INDEX(enemy_flag_item)]) {
                /*				gi.bprintf(PRINT_HIGH, "%s
                   captured the %s flag in %1.1 seconds!\n",
                                                                other->client->pers.netname,
                   CTFOtherTeamName(ctf_team),level.time-other->client->resp.ctf_flagsince);*/

                apply_time(other, ent);
                /*		other->client->resp.got_time = true;

                                other->client->resp.item_timer =
                   add_item_to_queue(other,other->client->resp.item_timer,other->client->resp.item_timer_penalty,other->client->pers.netname,enemy_flag_item->pickup_name);
                                if
                   ((other->client->resp.item_timer<level_items.item_time) ||
                   (level_items.item_time==0))
                                {
                                        level_items.jumps =
                   other->client->resp.jumps; level_items.item_time =
                   other->client->resp.item_timer;
                ...
                */
                ...
                //gi.soundindex("ctf/flagcap.wav"), 0.3, ATTN_NONE, 0);
        ...
        /*		// hey, its not home.  return it by teleporting it back
                        gi.bprintf(PRINT_HIGH, "%s returned the %s flag!\n",
                                other->client->pers.netname,
           CTFTeamName(ctf_team)); other->client->resp.score +=
           CTF_RECOVERY_BONUS; other->client->resp.ctf_lastreturnedflag =
           level.time; gi.sound (ent, CHAN_RELIABLE+CHAN_NO_PHS_ADD+CHAN_VOICE,
           gi.soundindex("ctf/flagret.wav"), 1, ATTN_NONE, 0);
                        //CTFResetFlag will remove this entity!  We must return
           false CTFResetFlag(ctf_team);*/
        return false;
    }

    // if we have the flag, dont spam again
    if (other->client->pers.inventory[ITEM_INDEX(flag_item)]) return false;
...
void CTFDeadDropFlag(edict_t *self) {
    ...
    /*	edict_t *dropped = NULL;
       ...
       CTF_AUTO_FLAG_RETURN_TIMEOUT; dropped->touch = CTFDropFlagTouch;
            }*/
}
...
void CTFEffects(edict_t *player) {
    ...
    if (player->client->resp.tagged) {
        ...
        } else if (gset_vars->transparent &&
                   player->client->resp.admin < aset_vars->ADMIN_MODEL_LEVEL) {
            player->s.renderfx |= RF_TRANSLUCENT;
        ...
        if (!number_of_jumpers_off) {
            if (gset_vars->glow_admin) {
                ...
                if (lvl > 0) {
                    player->s.effects |= EF_COLOR_SHELL;  // red
                    if (gset_vars->glow_multi && player->client->resp.admin ==
                                                     aset_vars->ADMIN_MAX_LEVEL)
                        player->s.renderfx |=
                            admin_colour_info[level.framenum % 15];
                    else
            ...
            /*		if (gset_vars->glow_time)
                            {
                                    if ((player->client->resp.got_time) &&
               (level_items.fastest_player!=player))
                                    {
                                            player->s.effects |= EF_COLOR_SHELL;
               // red player->s.renderfx |= RF_SHELL_DOUBLE;
                                    }
                            }
            */
            ...
            if (level_items.fastest_player == player) {
                /*			if (gset_vars->glow_fastest)
                                        {
                                                player->s.effects |=
                   EF_COLOR_SHELL; // red player->s.renderfx |= RF_SHELL_RED;
                                        }*/
...
static void CTFSetIDView(edict_t *ent) {
    ...
    for (i = 1; i <= maxclients->value; i++) {
        who = g_edicts + i;
        if (!who->inuse || who->solid == SOLID_NOT) continue;
    ...
    if (bd > 0.90) {
        ...
        /*		if (best->client->resp.ctf_team == CTF_TEAM1)
           ...
           imageindex_sbfctf1; else if (best->client->resp.ctf_team ==
           CTF_TEAM2) ent->client->ps.stats[STAT_CTF_ID_VIEW_COLOR] =
           imageindex_sbfctf2;*///pooy
    }
}
...
void SetCTFStats(edict_t *ent) {
    ...
    if (ent->client->resp.ctf_team == CTF_TEAM1 ||
        ...
        ent->client->ps.stats[STAT_JUMP_SPEED_MAX] =
            ent->client->resp.cur_speed;
    } else {
        if (ent->client->resp.replaying) {
            ent->client->ps.stats[STAT_HEALTH] = 0;
            ent->client->ps.stats[STAT_JUMP_SPEED_MAX] =
                ent->client->resp.rep_speed;
        } else {
    ...
    if (ctfgame.election != ELECT_NONE) {
        ent->client->ps.stats[STAT_JUMP_VOTE_REMAINING] =
        ...
        ent->client->ps.stats[STAT_JUMP_VOTE_INITIATED] =
            CONFIG_JUMP_VOTE_INITIATED;
    } else
        ent->client->ps.stats[STAT_JUMP_VOTE_INITIATED] = 0;

    if (!ent->client->resp.replaying) {
        if (ent->client->resp.key_forward) {
            ent->client->ps.stats[STAT_JUMP_KEY_BACK_FORWARD] =
                gi.imageindex("forward");
        } else if (ent->client->resp.key_back) {
            ent->client->ps.stats[STAT_JUMP_KEY_BACK_FORWARD] =
                gi.imageindex("back");
        } else {
        ...
        if (ent->client->resp.key_left) {
            ent->client->ps.stats[STAT_JUMP_KEY_LEFT_RIGHT] =
                gi.imageindex("left");
            ;
        ...
        ent->client->ps.stats[STAT_JUMP_FPS] =
            (keys & RECORD_FPS_MASK) >> RECORD_FPS_SHIFT;
    }
...
void CTFGrappleTouch(edict_t *self, edict_t *other, cplane_t *plane,
    ...
    if (self->owner->client->ctf_grapplestate != CTF_GRAPPLE_STATE_FLY) return;
...
void CTFGrapplePull(edict_t *self) {
    ...
    if (strcmp(self->owner->client->pers.weapon->classname, "weapon_grapple") ==
            0 &&
...
void JumpModScoreboardMessage(edict_t *ent, edict_t *killer) {
    ...
    for (i = 0; i < maxclients->value; i++) {
        ...
        if (gametype->value == GAME_CTF) {
            if (cl_ent->client->resp.ctf_team < CTF_TEAM1) continue;
        } else {
            if (cl_ent->client->resp.ctf_team != CTF_TEAM2) continue;
        }
        if (cl_ent->client->resp.uid > 0)
            score =
                cl_ent->client->resp.suid;  // cl_ent->client->resp.best_time;
        else
            score = -1;
        if (score < 0) score = 99998;
        score++;
        for (j = 0; j < total; j++) {
            if (score < sortedscores[j]) break;
        }
        for (k = total; k > j; k--) {
            sorted[k] = sorted[k - 1];
    ...
    for (i = 0; i < total; i++) {
        ...
        if (cl->resp.best_time) {
            Com_sprintf(
                ...
                tourney_record[trecid].time, tourney_record[trecid].completions,

                maplist.sorted_completions[cl->resp.suid].score,
                (float)maplist.sorted_completions[cl->resp.suid].score /
            ...
            if (cl->resp.uid > 0) {
                Com_sprintf(
                    entry, sizeof(entry),
                    "ctf %d %d %d %d %d xv 152 string \"  ------ ---- %4i  "
                    "%4.1f  %s\"",
                    -8, y, sorted[i], cl->ping, cl->resp.suid + 1,
                    maplist.sorted_completions[cl->resp.suid].score,
                    (float)maplist.sorted_completions[cl->resp.suid].score /
    ...
    if (gametype->value != GAME_CTF)
        for (i = 0; i < maxclients->value; i++) {
            ...
            if (cl->pers.idle_player ||
                cl->pers.frames_without_movement > 60000) {
                strcpy(teamstring, "Idle");
            } else {
    ...
    for (i = 0; i < maxclients->value; i++) {
        ...
        cl_ent = g_edicts + 1 + i;
        if (!cl_ent->inuse) continue;
        if (cl_ent->client->resp.ctf_team != CTF_NOTEAM) continue;
        if (cl_ent->client->pers.idle_player ||
            cl_ent->client->pers.frames_without_movement > 60000)
            idle = true;
    }
    total_specs = 0;
    for (i = 0; i < maxclients->value; i++) {
        ...
        if (cl->pers.idle_player ||
            cl->pers.frames_without_movement > 60000)  // add idle tag to chaser
        {
            Com_sprintf(entry, sizeof(entry), "xv %d yv %d string \" (idle)\"",
                        56 + (strlen(cl->pers.netname) * 8), y);
            j = strlen(entry);
            strcpy(string + stringlength, entry);
            stringlength += j;
        }
        ...
        if (cl->resp.replaying) {
            if (cl->resp.replaying == MAX_HIGHSCORES + 1)
                Com_sprintf(entry, sizeof(entry),
                            ...
                            -8, y, i, cl->ping, 0, idle ? 224 : 168);
            // =====================
            // show what you're replaying
            else if (cl->resp.replaying > MAX_HIGHSCORES + 1)
                Com_sprintf(
                    entry, sizeof(entry),
            ...
            Com_sprintf(
                entry, sizeof(entry),
                "ctf %d %d %d %d %d xv %d string \"%s%s\"", -8, y, i, cl->ping,
                0, idle ? 224 : 168, cl->chase_target ? " -> " : "",
    ...
    Com_sprintf(
        entry, sizeof(entry),
        "xv -16 yv %d string2 \"Next Maps (type nominate <map> or rand)\" yv "
        "%d stat_string 3 yv %d stat_string 5 yv %d stat_string 12 ",
        y, y + 16, y + 24, y + 32);
    j = strlen(entry);
    strcpy(string + stringlength, entry);
    stringlength += j;
...
void CTFHasTech(edict_t *who) {
    if (level.time - who->client->ctf_lasttechmsg > 2) {
        //		gi.centerprintf(who, "You already have a TECH
        //powerup.");
...
static edict_t *FindTechSpawn(void) {
    ...
    while (i--) spot = G_Find(spot, FOFS(classname), "info_player_deathmatch");
    if (!spot) spot = G_Find(spot, FOFS(classname), "info_player_deathmatch");
    return spot;
}
...
void CTFRespawnTech(edict_t *ent) {
    ...
    if ((spot = FindTechSpawn()) != NULL) SpawnTech(ent->item, spot);
    G_FreeEdict(ent);
}
...
void CTFResetTech(void) {
    ...
    for (ent = g_edicts + 1, i = 1; i < globals.num_edicts; i++, ent++) {
        if (ent->inuse)
            if (ent->item && (ent->item->flags & IT_TECH)) G_FreeEdict(ent);
    }
    SpawnTechs(NULL);
}
...
int CTFApplyResistance(edict_t *ent, int dmg) {
    ...
    if (ent->client && ent->client->silencer_shots) volume = 0.2;

    if (!tech) tech = FindItemByClassname("item_tech1");
...
int CTFApplyStrength(edict_t *ent, int dmg) {
    ...
    if (!tech) tech = FindItemByClassname("item_tech2");
    if (dmg && tech && ent->client &&
        ent->client->pers.inventory[ITEM_INDEX(tech)]) {
        return dmg * 2;
    }
...
qboolean CTFApplyHaste(edict_t *ent) {
    ...
    if (!tech) tech = FindItemByClassname("item_tech3");
    if (tech && ent->client && ent->client->pers.inventory[ITEM_INDEX(tech)])
        return true;
    return false;
}
...
void CTFApplyRegeneration(edict_t *ent) {
    ...
    if (!tech) tech = FindItemByClassname("item_tech4");
    if (tech && client->pers.inventory[ITEM_INDEX(tech)]) {
        ...
        if (noise && ent->client->ctf_techsndtime < level.time) {
            ent->client->ctf_techsndtime = level.time + 1;
            gi.sound(ent, CHAN_VOICE, gi.soundindex("ctf/tech4.wav"), volume,
                     ATTN_NORM, 0);
        }
    }
}
...
static void CTFSay_Team_Location(edict_t *who, char *buf) {
    ...
    while ((what = loc_findradius(what, who->s.origin, 1024)) != NULL) {
        ...
        for (i = 0; loc_names[i].classname; i++)
            if (strcmp(what->classname, loc_names[i].classname) == 0) break;
        if (!loc_names[i].classname) continue;
        ...
        if (hotsee && hotindex < loc_names[i].priority) continue;
        VectorSubtract(what->s.origin, who->s.origin, v);
    ...
    if (power_armor_type) {
        ...
        if (cells)
            sprintf(buf + strlen(buf), "%s with %i cells ",
                    (power_armor_type == POWER_ARMOR_SCREEN) ? "Power Screen"
                                                             : "Power Shield",
                    cells);
    }
...
qboolean CTFBeginElection(edict_t *ent, elect_t type, char *msg,
    ...
    if (type == ELECT_MAP || type == ELECT_ADDTIME || type == ELECT_NOMINATE ||
        type == ELECT_RAND || type == ELECT_DUMMY) {
    ...
    if (ent == NULL) {
        ctfgame.needvotes++;
        if (ctfgame.needvotes <= 0) ctfgame.needvotes = 1;
    }
    ...
    if (require_max)
        gi.bprintf(PRINT_HIGH,
                   "This is a MAX vote, so everyone not idle must vote YES for "
                   "it to pass.\n");
    else
        gi.bprintf(PRINT_HIGH, "Vote YES or NO on this request.\n");
    gi.bprintf(PRINT_HIGH, "Votes: %d  Needed: %d  Time left: %ds\n",
    ...
    gi.configstring(CONFIG_JUMP_VOTE_REMAINING,
                    va("%d seconds", (int)(ctfgame.electtime - level.time)));
    gi.configstring(CONFIG_JUMP_VOTE_CAST,
...
void CTFAssignGhost(edict_t *ent) {
    ...
    for (ghost = 0; ghost < MAX_CLIENTS; ghost++)
        if (!ctfgame.ghosts[ghost].code) break;
...
void CTFEndMatch(void) {
    ...
    if (ctfgame.team1 > ctfgame.team2)
        gi.bprintf(PRINT_CHAT,
                   "EASY team won over the HARD team by %d CAPTURES!\n",
                   ctfgame.team1 - ctfgame.team2);
    else if (ctfgame.team2 > ctfgame.team1)
        gi.bprintf(PRINT_CHAT,
                   "HARD team won over the EASY team by %d CAPTURES!\n",
                   ctfgame.team2 - ctfgame.team1);
    else if (ctfgame.total1 > ctfgame.total2)  // frag tie breaker
        gi.bprintf(PRINT_CHAT,
                   "EASY team won over the HARD team by %d POINTS!\n",
                   ctfgame.total1 - ctfgame.total2);
    else if (ctfgame.total2 > ctfgame.total1)
        gi.bprintf(PRINT_CHAT,
                   "HARD team won over the EASY team by %d POINTS!\n",
...
void CTFWinElection(int pvote, edict_t *pvoter) {
    ...
    if (ctfgame.etarget == NULL) {
        ...
        if (1 == pvote)
            msg = va("Vote passed, %s forced the vote.",
                     pvoter->client->pers.netname);
        else
    ...
    switch (ctfgame.election) {
        case ELECT_MATCH:
            ...
            if (competition->value < 3) gi.cvar_set("competition", "2");
            ctfgame.match = MATCH_SETUP;
            CTFResetAllPlayers();
            break;
        ...
        case ELECT_MAP:
            ...
            /*		for (i=0;i<maplist.nummaps;i++)
                                    if
               (strcmp(maplist.mapnames[i],level.mapname)==0)
                                    {
                                            UpdateTimes(i);
                                            break;
                                    }*/
            ...
            //		sort_users();
            // if (1 == pvote)
            // gi.bprintf(PRINT_HIGH, "Vote passed. %s forced the vote. Level
            // changing to %s.\n", 	pvoter->client->pers.netname,
            //ctfgame.elevel);
            //	msg = va("Vote passed, %s forced the vote.",
            //pvoter->client->pers.netname); else gi.bprintf(PRINT_HIGH, "%s's
            // vote has passed. Level changing to %s.\n",
        ...
        case ELECT_KICK:
            ...
            //		ctfgame.etarget->client->pers.netname,
            //ctfgame.ekick->client->pers.netname); 	msg = va("Vote passed, %s
            //forced the vote.", pvoter->client->pers.netname); else
            //	gi.bprintf(PRINT_HIGH, "%s's vote has passed. Kicking %s.\n",
            //		ctfgame.etarget->client->pers.netname,
            //ctfgame.ekick->client->pers.netname); 	msg = va("%s's vote has
            //passed.", ctfgame.etarget->client->pers.netname);
            gi.bprintf(PRINT_HIGH, "%s Kicking %s.\n", msg,
                       ctfgame.ekick->client->pers.netname);
            ...
            break;
        case ELECT_ADDTIME:
            // gi.bprintf(PRINT_HIGH, "%s's vote has passed. Adding %i
            // minutes.\n", 	ctfgame.etarget->client->pers.netname,
            //ctfgame.ekicknum);
            ...
            Update_Added_Time();
            timeleft =
                ((mset_vars->timelimit + map_added_time) -
                 ((int)(level.time /
                        60)));  // old ((int)(level.time / 60)) +
                                // (mset_vars->timelimit + map_added_time);
            ctfgame.election = ELECT_NONE;
            if (timeleft < 0) {
                //			gi.bprintf(PRINT_HIGH,"%d\n",((int)(level.time
                /// 60)) + (mset_vars->timelimit + map_added_time));
                End_Jumping();
                return;
            }
            break;
...
void CTFVoteYes(edict_t *ent) {
    ...
    if (ent->client->resp.voted) {
        gi.cprintf(ent, PRINT_HIGH,
                   "You already voted or were idle when vote was started.\n");
        return;
    }
    if (ctfgame.etarget != NULL) {
    ...
    if (ctfgame.electframe < ent->client->resp.enterframe) {
        gi.cprintf(ent, PRINT_HIGH,
                   "You cannot participate in a vote called prior to joining "
                   "the server.\n");
        return;
    }
    ...
    //gi.soundindex("misc/talk.wav"), 1, ATTN_NONE, 0);
...
void CTFVoteNo(edict_t *ent) {
    ...
    ent->client->resp.voted = true;
    //	gi.positioned_sound (world->s.origin, world, CHAN_AUTO | CHAN_RELIABLE,
    //gi.soundindex("jumpno.wav"), 1, ATTN_NONE, 0);
    ...
    if (ctfgame.require_max) {
        ctfgame.election = ELECT_NONE;
        gi.bprintf(PRINT_CHAT,
                   "Vote failed because a NO vote was issued when everyone was "
...
qboolean CTFMatchOn(void) {
    if (ctfgame.match == MATCH_GAME) return true;
    return false;
}
...
void CTFJoinTeam(edict_t *ent, int desired_team) {
    ...
    if (level.status == LEVEL_STATUS_OVERTIME) {
        if (ent->client->resp.ctf_team != CTF_NOTEAM) {
            gi.cprintf(ent, PRINT_HIGH,
                       "Cannot change teams during overtime\n");
            return;
        }
        if (level.overtime > gset_vars->overtimewait) {
            gi.cprintf(ent, PRINT_HIGH,
                       "Cannot join a team after countdown has ended\n");
            return;
        }
        can_join = true;
    ...
    if (desired_team == CTF_TEAM2 && gametype->value != GAME_CTF &&
        gset_vars->read_only_mode == 1) {
        gi.cprintf(ent, PRINT_HIGH,
                   "Hard team is currently disabled on this server\n");
        return;
    }
    ...
    if (ctfgame.match == MATCH_GAME) {
        if (ent->client->resp.ghost) ent->client->resp.ghost->code = 0;
        ent->client->resp.ghost = NULL;
        CTFAssignGhost(ent);
    }
    ...
    ent->client->resp.item_timer = 0;
    ent->client->resp.client_think_begin = 0;
    if (!level.status) Notify_Of_Team_Commands(ent);
...
void CTFJoinTeam1(edict_t *ent, pmenuhnd_t *p) { CTFJoinTeam(ent, CTF_TEAM1); }

void CTFJoinTeam2(edict_t *ent, pmenuhnd_t *p) { CTFJoinTeam(ent, CTF_TEAM2); }
...
void CTFChaseCam(edict_t *ent, pmenuhnd_t *p) {
    ...
    // =====================================
    // added by lilred
    if (ent->client->resp.replaying) ent->client->resp.replaying = 0;
    // =====================================
...
void CTFShowScores(edict_t *ent, pmenu_t *p) {
    ...
    if (ent->client->showscores > 2) ent->client->showscores = 0;
...
qboolean CTFStartClient(edict_t *ent) {
    if (ent->client->resp.ctf_team != CTF_NOTEAM) return false;
...
void CTFObserver(edict_t *ent) {
    ...
    if (ent->movetype == MOVETYPE_NOCLIP) CTFPlayerResetGrapple(ent);
    CTFDeadDropFlag(ent);
    // CTFDeadDropTech(ent);
...
qboolean CTFInMatch(void) {
    if (ctfgame.match > MATCH_NONE) return true;
    return false;
}
    ...
    if (ctfgame.match != MATCH_NONE) {
        ...
        if (t <= 0) {  // time ended on something
            switch (ctfgame.match) {
                ...
                case MATCH_GAME:
                    ...
                    gi.positioned_sound(
                        world->s.origin, world, CHAN_AUTO | CHAN_RELIABLE,
                        gi.soundindex("misc/bigtele.wav"), 1, ATTN_NONE, 0);
                    return false;
            }
        }
        ...
        switch (ctfgame.match) {
            case MATCH_SETUP:
                ...
                //			gi.configstring (CONFIG_CTF_MATCH,
                //text);
                break;
        ...
        if (warn_unbalanced->value) {
            ...
            if (team1 - team2 >= 2 && team2 >= 2) {
                if (ctfgame.warnactive != CTF_TEAM1) {
                    ...
                    if (gametype->value != GAME_CTF)
                        gi.configstring(CONFIG_CTF_TEAMINFO,
                                        "WARNING: EASY has too many players");
                    else
                        gi.configstring(CONFIG_CTF_TEAMINFO,
                ...
                if (ctfgame.warnactive != CTF_TEAM2) {
                    ...
                    if (gametype->value != GAME_CTF)
                        gi.configstring(CONFIG_CTF_TEAMINFO,
                                        "WARNING: HARD has too many players");
                    else
                        gi.configstring(CONFIG_CTF_TEAMINFO,
...
void CTFAdmin_SettingsApply(edict_t *ent, pmenuhnd_t *p) {
    ...
    /*	if (settings->instantweap != !!((int)instantweap->value)) {
                    gi.bprintf(PRINT_HIGH, "%s turned %s instant weapons.\n",
                            ent->client->pers.netname, settings->instantweap ?
       "on" : "off"); sprintf(st, "%d", (int)settings->instantweap);
...
void CTFAdmin_UpdateSettings(edict_t *ent, pmenuhnd_t *setmenu) {
    ...
    /*	sprintf(text, "Instant Weapons: %s", settings->instantweap ? "Yes" :
       "No"); PMenu_UpdateEntry(setmenu->entries + i, text, PMENU_ALIGN_LEFT,
       CTFAdmin_ChangeInstantWeap); i++;
    */
...
void CTFAdmin_Cancel(edict_t *ent, pmenuhnd_t *p) { PMenu_Close(ent); }
...
void CTFAdmin(edict_t *ent) {
    ...
    if (!allow_admin->value) {
        gi.cprintf(
            ent, PRINT_HIGH,
            "Administration is disabled (set allow_admin 1 to enable).\n");
        return;
    }
    ...
    if (gi.argc() > 1 && !ent->client->resp.admin) {
        ...
        if (alevel >= 0) {
            ...
            if (admin_pass[alevel].level <
                aset_vars->ADMIN_MAX_LEVEL)  // only print if below cap
...
void CTFStats(edict_t *ent) {
    ...
    *text = 0;
    ...
    if (i == MAX_CLIENTS) {
        if (*text) gi.cprintf(ent, PRINT_HIGH, "%s", text);
    ...
    *text = 0;
    for (i = 1; i <= maxclients->value; i++) {
        ...
        if (e2->client->chase_target) {
            sprintf(temp, "%s -> %s", e2->client->pers.netname,
        ...
        Com_sprintf(
            st, sizeof(st), "%3d %-32.32s %02d:%02d %4d %3d%s%s%s\n", i, temp,
            (level.framenum - e2->client->resp.enterframe) / 600,
            ((level.framenum - e2->client->resp.enterframe) % 600) / 10,
            e2->client->ping,
            (e2->client->resp.ctf_team == CTF_TEAM1) ? e2->client->resp.recalls
                                                     : e2->client->resp.score,
            (ctfgame.match == MATCH_SETUP || ctfgame.match == MATCH_PREGAME)
                ? (e2->client->resp.ready ? " (ready)" : " (notready)")
                : "",
            e2->client->resp.admin ? admin_lev : "",
            e2->client->resp.silence ? " Silenced" : "");
...
void CTFWarp(edict_t *ent) {
    ...
    if (gi.argc() < 2) {
        ...
        gi.cprintf(ent, PRINT_HIGH,
                   "mapvote notime - a random map with no times.\n");
        gi.cprintf(ent, PRINT_HIGH,
                   "mapvote todo - a map you have not done.\n");
        ...
        gi.cprintf(ent, PRINT_HIGH,
                   "mapvote skill [#] - map with a specific skill (1 to 5).\n");
        gi.cprintf(ent, PRINT_HIGH, "--------------------------------\n");
        return;
    }
    ...
    // next mapvote
        ...
        for (i = 0; i < gset_vars->maps_pass; i++) {
            if (Q_stricmp(maplist.mapnames[map], game.lastmaps[i]) == 0) {
                if (i == 0)
                    gi.cprintf(ent, PRINT_HIGH,
                               "You cannot vote that map because it is "
                               "currently being played.\n");
                else
                    gi.cprintf(ent, PRINT_HIGH,
                               "You cannot vote that map because it was played "
                               "%d map(s) ago.\n",
                               i);
    ...
    // new todo mapvote
        ...
        if (ent->client->resp.uid <= 0) {  // check for id
            gi.cprintf(ent, PRINT_HIGH,
                       "Join HARD team to load your identity.\n");
            return;
        }
        ...
        sprintf(text, "%s: Request to change map to %s (Newest to do)",
        ...
        if (CTFBeginElection(ent, ELECT_MAP, text, false)) {
            gi.configstring(
                ...
                HighAscii(va("Vote by %s", ent->client->pers.netname)));
            gi.configstring(CONFIG_JUMP_VOTE_TYPE,
                            va("Map: %s", maplist.mapnames[notimes[map]]));
            strncpy(ctfgame.elevel, maplist.mapnames[notimes[map]],
                    sizeof(ctfgame.elevel) - 1);
            if (ctfgame.needvotes == 0) CTFWinElection(0, NULL);
        }
    }
    ...
    // todo mapvote
        ...
        if (ent->client->resp.uid <= 0) {
            gi.cprintf(ent, PRINT_HIGH,
                       "You have not completed ANY maps. Join HARD team to "
                       "load your identity.\n");
            return;
        }
    ...
    // notime mapvote
        ...
        for (i = 0; i < gset_vars->maps_pass; i++) {
            if (Q_stricmp(maplist.mapnames[notimes[map]], game.lastmaps[i]) ==
                0) {
                if (i == 0)
                    gi.cprintf(ent, PRINT_HIGH,
                               "You cannot vote that map because it is "
    ...
    // voting by mapname or id
        ...
        if (map == -1) {
            ...
            if ((temp_num >= 1) && (temp_num <= maplist.nummaps)) {
                ...
                gi.cprintf(ent, PRINT_HIGH,
                           "Unknown map, for available maps type maplist.\n");
                return;
            }
        }
...
void CTFBoot(edict_t *ent) {
    ...
    if ((!map_allow_voting) &&
        (ent->client->resp.admin < aset_vars->ADMIN_BOOT_LEVEL))
        return;
    ...
    if (ent->client->resp.admin > 1) {
        if (targ->client->resp.admin >= ent->client->resp.admin) {
            gi.cprintf(ent, PRINT_HIGH,
                       "You may not boot an admin with a level higher or equal "
                       "to yours.\n");
            return;
        }
    }
    ...
    if ((ClientIsBanned(ent, BAN_BOOT)) &&
        (ent->client->resp.admin < aset_vars->ADMIN_BOOT_LEVEL)) {
        gi.cprintf(ent, PRINT_HIGH,
                   "You are not allowed to vote to kick players.\n");
        return;
    }
    ...
    sprintf(text, "%s has requested kicking %s.", ent->client->pers.netname,
            targ->client->pers.netname);
    if (CTFBeginElection(ent, ELECT_KICK, text, false)) {
        gi.configstring(CONFIG_JUMP_VOTE_INITIATED,
                        HighAscii(va("Vote by %s", ent->client->pers.netname)));
        gi.configstring(CONFIG_JUMP_VOTE_TYPE,
                        va("Kick: %s", targ->client->pers.netname));
...
void sort_votemaps(void) {
    ...
    for (i = 0; i < maplist.nummaps - 1; ++i)
        for (j = 1; j < maplist.nummaps - i; ++j)
            if (maplist.update[votemaplist[j - 1]] >
                maplist.update[votemaplist[j]]) {
...
void GenerateVoteMaps(void) {
    ...
    if (maplist.nummaps <= 0) {
        printf(
            "Error: Cannot generate vote maps.  Number of maps is <= 0 "
            "(actual: %d)\n",
            maplist.nummaps);
        return;
    }
...
void CTFUpdateVoteMenu(edict_t *ent, pmenuhnd_t *p) {
    ...
    int index;
    // index = ent-g_edicts-1;
    sprintf(text, "*%2d %s[No Vote]", vote_data.votes[0],
    ...
    if (ent->client->resp.uid > 0) {
        ...
        if (overall_completions[index].maps[map1] == 1) map1_done[0] = 'Y';
        if (overall_completions[index].maps[map2] == 1) map2_done[0] = 'Y';
        if (overall_completions[index].maps[map3] == 1) map3_done[0] = 'Y';
    }

    sprintf(text, "*%2d %s%-16s%5s%s", vote_data.votes[1],
            (ent->client->resp.current_vote == 1 ? "*" : " "),
            maplist.mapnames[vote_data.maps[0]], vote_data.data[0].skill,
            map1_done);
    PMenu_UpdateEntry(p->entries + 5, text, PMENU_ALIGN_LEFT, CTFVoteChoice1);

    sprintf(text, "*%2d %s%-16s%5s%s", vote_data.votes[2],
            (ent->client->resp.current_vote == 2 ? "*" : " "),
            maplist.mapnames[vote_data.maps[1]], vote_data.data[1].skill,
            map2_done);
    PMenu_UpdateEntry(p->entries + 8, text, PMENU_ALIGN_LEFT, CTFVoteChoice2);

    sprintf(text, "*%2d %s%-16s%5s%s", vote_data.votes[3],
            (ent->client->resp.current_vote == 3 ? "*" : " "),
            maplist.mapnames[vote_data.maps[2]], vote_data.data[2].skill,
            map3_done);
    PMenu_UpdateEntry(p->entries + 11, text, PMENU_ALIGN_LEFT, CTFVoteChoice3);
    time_left = gset_vars->votingtime - level.votingtime;

    if (vote_data.data[0].time) {
        sprintf(text, "    %-16s%6.1f", vote_data.data[0].name,
                vote_data.data[0].time);
    ...
    if (vote_data.data[1].time) {
        sprintf(text, "    %-16s%6.1f", vote_data.data[1].name,
                vote_data.data[1].time);
    ...
    if (vote_data.data[2].time) {
        sprintf(text, "    %-16s%6.1f", vote_data.data[2].name,
                vote_data.data[2].time);
...
void CTFHelp_main(edict_t *ent, pmenuhnd_t *p) {
    PMenu_Close(ent);
    PMenu_Open(ent, helpmenu_main, -1, sizeof(helpmenu_main) / sizeof(pmenu_t),
               NULL);
}

void CTFHelp_misc(edict_t *ent, pmenuhnd_t *p) {
    PMenu_Close(ent);
    PMenu_Open(ent, helpmenu_misc, -1, sizeof(helpmenu_misc) / sizeof(pmenu_t),
               NULL);
}

void CTFHelp_vote(edict_t *ent, pmenuhnd_t *p) {
    PMenu_Close(ent);
    PMenu_Open(ent, helpmenu_vote, -1, sizeof(helpmenu_vote) / sizeof(pmenu_t),
               NULL);
}
...
int Get_Voting_Clients(void) {
    ...
    for (i = 1; i <= maxclients->value; i++) {
        ...
        if (e->inuse) {
            if (e->client->pers.frames_without_movement > 60000 ||
                e->client->pers.idle_player) {

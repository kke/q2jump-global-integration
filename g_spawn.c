...
spawn_t spawns[] = {{"jump_clip", SP_jump_clip},
...
char *ED_ParseEdict(char *data, edict_t *ent, int add) {
    ...
    while (1) {
        ...
        if (!data) {
            break_out_of_spawn = true;
            break;
            //			gi.error ("ED_ParseEntity: EOF without closing
            //brace");
        }
        if (com_token[0] == '}') {
            break_out_of_spawn = true;
            break;
            //			gi.error ("ED_ParseEntity: closing brace without
            //data");
        }
...
void SpawnEntities(char *mapname, char *entities, char *spawnpoint) {
    ...
    if (removed_map) {
        //		gi.cprintf(ent,PRINT_HIGH,"A map has been removed so
    ...
    for (i = 0; i < 10; i++) level.jumpboxes[i] = 0;
...
done2:
    ...
    while (1) {
        ...
        if (ent != g_edicts) {
            ...
            if ((strstr(ent->classname, "item_flag_team2")) ||
    ...
    if ((fe_add != NULL) && (buffer2 != NULL))
        while (1) {
            ...
            if (com_token[0] != '{') {
                ...
                //				G_FreeEdict (ent);
                break;
                //			gi.error ("ED_LoadFromFile: found %s
                //when expecting {",com_token);
            }
            if (!ent)
                ent = g_edicts;
            else
    ...
    */
    ...
    if (level_items.recorded_time_frames[0] &&
        level_items.stored_item_times[0].time > 0)
    ...
    if (gset_vars->global_integration_enabled == 1) {
        ...
        // clear the array records so old cached stuff doesn't carry through to
        // next map
...
void SP_worldspawn(edict_t *ent) {
    ...
    if (st.nextmap) strcpy(level.nextmap, st.nextmap);
    ...
    gi.configstring(CS_SKYAXIS, va("%f %f %f", st.skyaxis[0], st.skyaxis[1],
                                   st.skyaxis[2]));
    ...
    for (i = 2; i >= 0; i--) {
        Com_sprintf(prev_levels[i + 1].mapname,
                    sizeof(prev_levels[i + 1].mapname), "%16s",
                    prev_levels[i].mapname);
    }
    if (strlen(this_map) > 16)
        for (i = 16; i < 64; i++) this_map[i] = 0;
    ...
    if (mset_vars->ghost_model > 0 &&
        ghost_model_list[mset_vars->ghost_model].name[0])
        gi.modelindex(va("players/ghost/%s.md2",

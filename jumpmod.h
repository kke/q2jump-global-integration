...
#define MAX_REMOTE_HOSTS \
    5  // DO NOT EDIT!! unless you also update the global_* gsets
...
#define BAN_TEMPADMIN \
    128  // Client will not automatically receive temporary admin when 5 minutes
         // of a game is remaining
#define BAN_MOVE 256  // Client cannot move :D
#define BAN_PLAY 512  // Client can spectate but not play
#define BAN_KICK_BAN \
    1024  // Same as BAN_CONNECTION but player is told they will be allowed back
          // next map
...
typedef struct {
    ...
    // make arrays +MAX_REMOTE_REPLAYS for global support (from
    // 1+MAX_HIGHSCORES)
    ...
    record_data recorded_time_data[1 + MAX_HIGHSCORES + MAX_REMOTE_REPLAYS]
                                  [MAX_RECORD_FRAMES];
...
qboolean ValidateMap(
    char *mapname);  // updated to try and download a missing map
...
static const double replay_speed_modifier[] = {
    -100, -25, -10, -5, -2, -1, -0.5, -0.2, -0.1, 0,
    0.1,  0.2, 0.5, 1,  2,  5,  10,   25,   100};

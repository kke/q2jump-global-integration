...
void P_DamageFeedback(edict_t *player) {
    ...
    realcount = count;
    if (count < 10) count = 10;  // allways make a visible effect
    ...
    if ((level.time > player->pain_debounce_time) &&
        ...
        if (player->health < 25)
            ...
            l = 100;
        // gi.sound (player, CHAN_VOICE, gi.soundindex(va("*pain%i_%i.wav", l,
        // r)), 1, ATTN_NORM, 0);
    }
...
void SV_CalcBlend(edict_t *ent) {
    ...
    if (Jet_Active(ent)) {
        ...
        if (ent->client->Jet_remaining > 40 ||
            ((int)ent->client->Jet_remaining & 4))
    ...
    if (ent->client->damage_alpha < 0) ent->client->damage_alpha = 0;

    // drop the bonus value
    ent->client->bonus_alpha -= 0.1;
    if (ent->client->bonus_alpha < 0) ent->client->bonus_alpha = 0;
}
...
void P_FallingDamage(edict_t *ent) {
    ...
    if (ent->client->hook_state == HOOK_ON) return;
    ...
    if (delta < 15) {
        jumpmod_sound(ent, false,
                      gi.soundindex(va("player/step%i.wav", (rand() % 4) + 1)),
                      CHAN_BODY, 1, ATTN_NORM);  // jumpmod footsteps
...
void P_WorldEffects(void) {
    ...
    if (!old_waterlevel && waterlevel) {
        ...
        if (current_player->watertype & CONTENTS_SLIME) {
            ...
            jumpmod_sound(current_player, false,
                          gi.soundindex("player/watr_in.wav"), CHAN_BODY, 1,
                          ATTN_NORM);
    ...
    if (old_waterlevel && !waterlevel) {
        ...
        jumpmod_sound(current_player, false,
                      gi.soundindex("player/watr_out.wav"), CHAN_BODY, 1,
                      ATTN_NORM);
        current_player->flags &= ~FL_INWATER;
    }
    ...
    if (old_waterlevel != 3 && waterlevel == 3) {
        ...
        jumpmod_sound(current_player, false,
                      gi.soundindex("player/watr_un.wav"), CHAN_BODY, 1,
                      ATTN_NORM);
    }
    ...
    if (waterlevel == 3) {
        ...
        if (Jet_Active(current_player)) /*dont jet and dive and stay alive*/
        {
            ...
            player_die(current_player, current_player, current_player, 100000,
                       vec3_origin);
        }
        /*ATTILA end*/
        ...
        if (current_player->air_finished < level.time) {  // drown!
            if (current_player->client->next_drown_time < level.time &&
                ...
                if (current_player->dmg > 15) current_player->dmg = 15;
                ...
                if (current_player->health <= current_player->dmg)
                    ...
                    jumpmod_sound(current_player, false,
                                  ...
                                  CHAN_VOICE, 1, ATTN_NORM);
                else if (rand() & 1)
                    // gi.sound(current_player, CHAN_VOICE,
                    // gi.soundindex("*gurp1.wav"), 1, ATTN_NORM, 0);
                    jumpmod_sound(current_player, false,
                                  gi.soundindex("*gurp1.wav"), CHAN_VOICE, 1,
                                  ATTN_NORM);
                else
                    // gi.sound(current_player, CHAN_VOICE,
                    // gi.soundindex("*gurp2.wav"), 1, ATTN_NORM, 0);
                    jumpmod_sound(current_player, false,
                                  gi.soundindex("*gurp2.wav"), CHAN_VOICE, 1,
                                  ATTN_NORM);
    ...
    if (waterlevel &&
        ...
        if (current_player->watertype & CONTENTS_LAVA) {
            if (current_player->health > 0 &&
                ...
                //gi.soundindex("player/burn1.wav"), 1, ATTN_NORM, 0); 				else
                //					gi.sound
                //(current_player, CHAN_VOICE,
...
void G_SetClientFrame(edict_t *ent) {
    ...
    if (run != client->anim_run && client->anim_priority == ANIM_BASIC)
        goto newanim;
    if (!ent->groundentity && client->anim_priority <= ANIM_WAVE) goto newanim;
    ...
    if (client->anim_priority == ANIM_JUMP) {
        if (!ent->groundentity) return;  // stay there
...
void ClientEndServerFrame(edict_t *ent) {
    ...
    //	if (1 == ESF_debug)
    //		debug_log ("CHECKPOINT: Function: ClientEndServerFrame Line:
    //1058 File: p_view.c");
    ...
    if (current_client->ps.pmove.pm_flags & PMF_DUCKED) bobtime *= 4;

    bobcycle = (int)bobtime;
    bobfracsin = fabs(sin(bobtime * M_PI));

    // detect hitting the floor
    if (mset_vars->falldamage) P_FallingDamage(ent);
    ...
    for (i = 1; i <= maxclients->value; i++) {
        ...
        if (!e->inuse || e->client->chase_target != ent) continue;
        ...
        //		break; // LilRed: This is the fix to the chasecam bug...
        //commenting out break; =)
    }
    // ZOID
    ...
    if (!ent->client->resp.paused) ent->client->resp.race_frame++;
    ...
    if (ent->client->resp.race_frame >=
        level_items.recorded_time_frames[race_this]) {

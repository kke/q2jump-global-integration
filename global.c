...
char *pGlobalHostName[5] = {gset_vars->global_name_1, gset_vars->global_name_2,
                            gset_vars->global_name_3, gset_vars->global_name_4,
                            gset_vars->global_name_5};
char *pGlobalHostUrl[5] = {gset_vars->global_url_1, gset_vars->global_url_2,
                           gset_vars->global_url_3, gset_vars->global_url_4,
                           gset_vars->global_url_5};
int *pGlobalHostPort[5] = {&gset_vars->global_port_1, &gset_vars->global_port_2,
                           &gset_vars->global_port_3, &gset_vars->global_port_4,
...
1) Update the vars above ie. char *pGlobalHostName[5] =
{...,gset_vars->global_name_5}; etc. 2) Add another set of gset_cvars. Look for
"global_*" gset_cvars in jumpmod.c & jumpmod.h 3) Increment "MAX_REMOTE_HOSTS"
in jumpmod.h

Note: Using static vars per "set" of remote host vars in lieu of one single
array, simply to make it easier for admins to configure hosts via
gsets/jump_mod.cfg
======================================================================================
*/
...
remote_map_best_times_record remote_map_best_times[MAX_REMOTE_HOSTS]
                                                  [MAX_HIGHSCORES];
...
allowing for preservation of an existing file if the replacement file is
empty/unavailable
==================
*/
...
This will add up and return the received byte chunks so the calling function can
...
void Download_Remote_Mapents(char *mapname) {
    ...
    sprintf(args[0].url, "%s/mapsent/%s.ent", gset_vars->global_ents_url,
            urlencoded_mapname);
    ...
    sprintf(args[1].url, "%s/ent/%s.cfg", gset_vars->global_ents_url,
            urlencoded_mapname);
    ...
    sprintf(args[2].url, "%s/ent/%s.add", gset_vars->global_ents_url,
            urlencoded_mapname);
...
void Download_Remote_Recordings_NB() {
    // validations
    if (gset_vars->global_integration_enabled == 0) return;

    if (gset_vars->global_replay_max == 0) return;

    if (gset_vars->global_replay_max > MAX_REMOTE_REPLAYS) return;
    ...
    for (i = 0; i < gset_vars->global_replay_max; i++) {
        if (strcmp(sorted_remote_map_best_times[i].server,
                   gset_vars->global_localhost_name) == 0)
            continue;  // local record, no download needed

        if (sorted_remote_map_best_times[i].time > 0.0001) {
            sprintf(url, "%s/jumpdemo/%s_%d.dj3",
                    sorted_remote_map_best_times[i].replay_host_url,
                    level.mapname, sorted_remote_map_best_times[i].id);
            sprintf(filename, "%s/global/jumpdemo/%s_%d.%s", tgame->string,
                    level.mapname, sorted_remote_map_best_times[i].id,
                    sorted_remote_map_best_times[i].server);
            fp[total_handles] = Open_File_RB_WB(filename);
            if (!fp[total_handles]) continue;
            handles[total_handles] = curl_easy_init();
            curl_easy_setopt(handles[total_handles], CURLOPT_URL, url);
            curl_easy_setopt(handles[total_handles], CURLOPT_WRITEDATA,
                             fp[total_handles]);
            curl_easy_setopt(handles[total_handles], CURLOPT_FAILONERROR, 1L);
            curl_easy_setopt(handles[total_handles], CURLOPT_TIMEOUT,
                             HTTP_MULTI_TIMEOUT);
            curl_easy_setopt(handles[total_handles], CURLOPT_CONNECTTIMEOUT,
                             HTTP_MULTI_CONN_TIMEOUT);
            curl_easy_setopt(handles[total_handles], CURLOPT_SSL_VERIFYHOST,
                             0L);
            curl_easy_setopt(handles[total_handles], CURLOPT_SSL_VERIFYPEER,
                             0L);
            total_handles++;
        }
    }
...
void Download_Remote_Recordings_MT(char *mapname) {
    ...
    curl_global_init(CURL_GLOBAL_ALL);
    CURL
        *tmp_curl;  // just to url encode the mapname (thanks railjump#1 etc..!)
    tmp_curl = curl_easy_init();
    if (tmp_curl) {
        urlencoded_mapname = curl_easy_escape(tmp_curl, mapname, 0);
    }
    ...
    for (i = 0; i < gset_vars->global_replay_max; i++) {
        ...
        if (sorted_remote_map_best_times[i].time > 0.0001) {
            sprintf(args[i].url, "%s/jumpdemo/%s_%d.dj3",
                    sorted_remote_map_best_times[i].replay_host_url,
                    urlencoded_mapname, sorted_remote_map_best_times[i].id);
            sprintf(args[i].filename, "%s/global/jumpdemo/%s_%d.%s",
                    tgame->string, mapname, sorted_remote_map_best_times[i].id,
                    sorted_remote_map_best_times[i].server);
            ...
            pthread_create(&tid[tcount], NULL, HTTP_Get_File_MT,
                           (void *)&args[i]);
            tcount++;
        }
    }
    ...
    for (i = 0; i < gset_vars->global_replay_max; i++) {
        ...
        if (sorted_remote_map_best_times[i].time > 0.0001) {
            sprintf(url, "%s/jumpdemo/%s_%d.dj3",
                    sorted_remote_map_best_times[i].replay_host_url,
                    level.mapname, sorted_remote_map_best_times[i].id);
            sprintf(demo_file_path, "%s/global/jumpdemo/%s_%d.%s",
                    tgame->string, level.mapname,
                    sorted_remote_map_best_times[i].id,
                    sorted_remote_map_best_times[i].server);
...
void Load_Remote_Recordings(int index_from) {
    ...
    // index_from arg is used to reload demos from index_now position, handy for
    // when a new time is set locally
    if (index_from > 0) index = index_from;

    if (gset_vars->global_replay_max > MAX_REMOTE_REPLAYS ||
        index >= gset_vars->global_replay_max)
        return;  // above max limit, exit

    for (i = index; i < gset_vars->global_replay_max; i++) {
        level_items.recorded_time_frames[MAX_HIGHSCORES + (i + 1)] =
            0;  // clear old slot
        if (strcmp(sorted_remote_map_best_times[i].server,
                   gset_vars->global_localhost_name) == 0)  // local demo
        {
            sprintf(demo_file_path, "%s/jumpdemo/%s_%d.dj3", tgame->string,
                    level.mapname, sorted_remote_map_best_times[i].id);
        } else if (sorted_remote_map_best_times[i].time >
                   0.0001)  // remote demo
        {
            sprintf(demo_file_path, "%s/global/jumpdemo/%s_%d.%s",
                    tgame->string, level.mapname,
        ...
        fread(level_items.recorded_time_data[MAX_HIGHSCORES + (i + 1)], 1,
              lSize, f);
        level_items.recorded_time_frames[MAX_HIGHSCORES + (i + 1)] =
            lSize / sizeof(record_data);
        fclose(f);
    }
}
...
void Download_Remote_Maptimes(char *mapname) {
    ...
    for (i = 0; i < MAX_REMOTE_HOSTS; i++) {
        ...
        if (strlen(pGlobalHostName[i]) < 1 ||
            strcmp(pGlobalHostName[i], "default") == 0 ||
            strlen(pGlobalHostUrl[i]) < 12)
            continue;

        for (j = i + 1; j < MAX_REMOTE_HOSTS; j++) {
            if ((strcmp(pGlobalHostUrl[i], pGlobalHostUrl[j]) == 0) &&
                (*pGlobalHostPort[i] == *pGlobalHostPort[j])) {
        ...
        sprintf(args[i].filename, "%s/global/maptimes/%s.t.%s", tgame->string,
                mapname, pGlobalHostName[i]);
        sprintf(args[i].url, "%s/%d/%s.t", pGlobalHostUrl[i],
                *pGlobalHostPort[i], urlencoded_mapname);
        if (tcount == gset_vars->global_threads_max) {
            for (j = 0; j < tcount; j++) {
                pthread_join(tid[j], NULL);  // close threads
            }
...
void Download_Remote_Users_Files() {
    ...
    for (i = 0; i < MAX_REMOTE_HOSTS; i++) {
        ...
        if (strlen(pGlobalHostName[i]) < 1 ||
            ...
            continue;
        {
            sprintf(args[i].url, "%s/%d/users.t", pGlobalHostUrl[i],
                    *pGlobalHostPort[i]);
            sprintf(args[i].filename, "%s/global/maptimes/users.t.%s",
                    tgame->string, pGlobalHostName[i]);
            if (tcount == gset_vars->global_threads_max) {
                for (j = 0; j < tcount; j++) {
                    pthread_join(tid[j], NULL);  // close threads
                }
...
void Load_Remote_Users_Files() {
    ...
    for (i = 0; i < MAX_REMOTE_HOSTS; i++) {
        sprintf(rusers_file, "%s/global/maptimes/users.t.%s", tgame->string,
                pGlobalHostName[i]);
...
void Load_Remote_Maptimes(char *mapname) {
    ...
    for (i = 0; i < MAX_REMOTE_HOSTS; i++) {
        sprintf(rmap_file, "%s/global/maptimes/%s.t.%s", tgame->string, mapname,
                pGlobalHostName[i]);
        ...
        // load file
        while (fscanf(f, "%s %f %d %d", rdate, &rmaptime, &ruid,
                      &rcompletions) != EOF) {
            ...
            for (k = 0; k < MAX_USERS; k++) {
                if (ruid == remote_users[i][k].id) {
                    strcpy(remote_map_best_times[i][j].name,
                           remote_users[i][k].name);
...
Re-organises the global replay data after a new top15 time has been set during
gameplay
==============================
*/
    ...
    for (i = 0; i < gset_vars->global_replay_max; i++) {
        if (item_time < sorted_remote_map_best_times[i].time ||
            ...
            if (i == gset_vars->global_replay_max -
                         1)  // beat last global replay spot, just replace it
                             // and get out
            {
                level_items.recorded_time_frames[MAX_HIGHSCORES + (i + 1)] =
                    client_record[index].current_frame;
                memcpy(level_items.recorded_time_data[MAX_HIGHSCORES + (i + 1)],
                       client_record[index].data,
            ...
            } else  // shift any others down the list... before overwriting...
            {
                for (j = gset_vars->global_replay_max; j > i; j--) {
                    if (level_items
                            .recorded_time_frames[MAX_HIGHSCORES + (j - 1)]) {
                        gi.dprintf(
                            "Shifting global demos, rec time frames j-1 is: %d "
                            "i is: %d, j is: %d\n",
                            level_items
                                .recorded_time_frames[MAX_HIGHSCORES + (j - 1)],
                            i, j);  // log only
                        level_items.recorded_time_frames[MAX_HIGHSCORES + j] =
                            level_items
                                .recorded_time_frames[MAX_HIGHSCORES + (j - 1)];
                        memcpy(
                            level_items.recorded_time_data[MAX_HIGHSCORES + j],
                            level_items
                                .recorded_time_data[MAX_HIGHSCORES + (j - 1)],
                            sizeof(
                                level_items.recorded_time_data[MAX_HIGHSCORES +
                                                               (j - 1)]));
                    }
                }
                level_items.recorded_time_frames[MAX_HIGHSCORES + (i + 1)] =
...
void Sort_Remote_Maptimes() {
    ...
    memset(sorted_remote_map_best_times, 0,
           sizeof(sorted_remote_map_best_times));

    // MERGE all remote arrays (plus local server) into one...
    for (i = 0; i < MAX_REMOTE_HOSTS; i++) {
        for (j = 0; j < MAX_HIGHSCORES; j++) {
            if (remote_map_best_times[i][j].time > 0 &&
                strlen(remote_map_best_times[i][j].name) > 0) {
                sorted_remote_map_best_times[index].id =
                    remote_map_best_times[i][j].id;
                strcpy(sorted_remote_map_best_times[index].name,
                       remote_map_best_times[i][j].name);
                sorted_remote_map_best_times[index].time =
                    remote_map_best_times[i][j].time;
                strcpy(sorted_remote_map_best_times[index].date,
                       remote_map_best_times[i][j].date);
                sorted_remote_map_best_times[index].completions =
                    remote_map_best_times[i][j].completions;
                strcpy(sorted_remote_map_best_times[index].server,
                       pGlobalHostName[i]);
                strcpy(sorted_remote_map_best_times[index].replay_host_url,
                       pGlobalHostUrl[i]);
                index++;
            } else
    ...
    for (j = 0; j < (MAX_HIGHSCORES * MAX_REMOTE_HOSTS) + MAX_HIGHSCORES; j++) {
        for (i = 0; i < (MAX_HIGHSCORES * MAX_REMOTE_HOSTS) + MAX_HIGHSCORES;
             i++) {
            if (sorted_remote_map_best_times[i].time > 0) {
                if (sorted_remote_map_best_times[i].time >
                        sorted_remote_map_best_times[i + 1].time &&
                    sorted_remote_map_best_times[i + 1].time > 0) {
                    ...
                    // now do the swapping!
                    sorted_remote_map_best_times[i].id =
                        sorted_remote_map_best_times[i + 1].id;
                    sorted_remote_map_best_times[i].time =
                        sorted_remote_map_best_times[i + 1].time;
                    sorted_remote_map_best_times[i].completions =
                        sorted_remote_map_best_times[i + 1].completions;
                    strcpy(sorted_remote_map_best_times[i].name,
                           sorted_remote_map_best_times[i + 1].name);
                    strcpy(sorted_remote_map_best_times[i].date,
                           sorted_remote_map_best_times[i + 1].date);
                    strcpy(sorted_remote_map_best_times[i].server,
                           sorted_remote_map_best_times[i + 1].server);
                    strcpy(sorted_remote_map_best_times[i].replay_host_url,
                           sorted_remote_map_best_times[i + 1].replay_host_url);
                    // swap part 2...
                    ...
                    strcpy(sorted_remote_map_best_times[i + 1].replay_host_url,
                           url);
                }
            }
        }
    ...
    for (i = 0; i < size; i++) {
        for (j = i + 1; j < size; j++) {
            if (strcmp(sorted_remote_map_best_times[i].name,
                       sorted_remote_map_best_times[j].name) == 0) {
                // overrite the dupe with next record in line...
                for (k = j; k < size - 1; k++) {
                    sorted_remote_map_best_times[k].id =
                        sorted_remote_map_best_times[k + 1].id;
                    sorted_remote_map_best_times[k].time =
                        sorted_remote_map_best_times[k + 1].time;
                    sorted_remote_map_best_times[k].completions =
                        sorted_remote_map_best_times[k + 1].completions;
                    strcpy(sorted_remote_map_best_times[k].name,
                           sorted_remote_map_best_times[k + 1].name);
                    strcpy(sorted_remote_map_best_times[k].date,
                           sorted_remote_map_best_times[k + 1].date);
                    strcpy(sorted_remote_map_best_times[k].server,
                           sorted_remote_map_best_times[k + 1].server);
                    strcpy(sorted_remote_map_best_times[k].replay_host_url,
                           sorted_remote_map_best_times[k + 1].replay_host_url);
                }
                size--;
                j--;
            }
...
void Print_Sorted_Maptimes(edict_t *ent) {
    ...
    gi.cprintf(
        ...
        "\xd4\xe9\xed\xe5       \xd3\xe5\xf2\xf6\xe5\xf2\n");  // No. Name Date
                                                               // Time Server
    ...
    for (i = 0; i < MAX_HIGHSCORES * 2; i++) {
        ...
        Com_sprintf(this_name, sizeof(this_name),
                    sorted_remote_map_best_times[i].name);
        Highlight_Name(this_name);
        Com_sprintf(this_server, sizeof(this_server),
                    sorted_remote_map_best_times[i].server);
        if (strcmp(sorted_remote_map_best_times[i].server,
                   gset_vars->global_localhost_name) == 0) {
        ...
        gi.cprintf(ent, PRINT_HIGH, "%-3d %-18s %8s    %-9.3f  %-s\n", count,
                   this_name, sorted_remote_map_best_times[i].date,
...
void Print_Remote_Maptimes(edict_t *ent, char *server) {
    ...
    if (tourney_record[0].time > 0) localfound = 1;

    // prep fancy font
    Com_sprintf(wr_txt, sizeof(wr_txt), "wr");

    for (i = 0; i < MAX_REMOTE_HOSTS; i++) {
        if (strcmp(server, pGlobalHostName[i]) == 0 ||
            atoi(server) == (i + 2)) {
            if (remote_map_best_times[i][0].time == 0) {
                gi.cprintf(ent, PRINT_HIGH,
                           "No times found for remote server %s\n",
                           pGlobalHostName[i]);
                return;
            } else {
                gi.cprintf(ent, PRINT_HIGH,
                ...
                gi.cprintf(
                    ent, PRINT_HIGH,
                    "\xce\xef\xae     \xce\xe1\xed\xe5               "
                    "\xc4\xe1\xf4\xe5        \xd4\xe9\xed\xe5       "
                    "\xc3\xef\xed\xf0\xec\xe5\xf4\xe9\xef\xee\xf3\n");  // No.
                                                                        // Name
                                                                        // Date
                                                                        // Time
                                                                        // Completions
                ...
                for (j = 0; j < MAX_HIGHSCORES; j++) {
                    if (remote_map_best_times[i][j].time == 0) break;

                    // highlight connected user
                    Com_sprintf(this_name, sizeof(this_name),
                                remote_map_best_times[i][j].name);
                    Highlight_Name(this_name);
                    if (remote_map_best_times[i][j].time ==
                        sorted_remote_map_best_times[0].time)  // wr time
                    {
                        gi.cprintf(ent, PRINT_HIGH,
                                   "%-3d (%s)%-18s %8s    %-9.3f  %d\n", 1,
                                   HighAscii(wr_txt), this_name,
                                   remote_map_best_times[i][j].date,
                                   remote_map_best_times[i][j].time,
                                   remote_map_best_times[i][j].completions);
                    } else
                        gi.cprintf(ent, PRINT_HIGH,
                                   "%-3d     %-18s %8s    %-9.3f  %d\n", j + 1,
                                   this_name, remote_map_best_times[i][j].date,
                                   remote_map_best_times[i][j].time,
                                   remote_map_best_times[i][j].completions);
                }
    ...
    // gi.cprintf(ent, PRINT_HIGH, "ID  Server    Top Player         Date Time
    // Compl.\n");
    gi.cprintf(ent, PRINT_HIGH,
               "\xc9\xc4  \xd3\xe5\xf2\xf6\xe5\xf2    \xd4\xef\xf0 "
               "\xd0\xec\xe1\xf9\xe5\xf2         \xc4\xe1\xf4\xe5        "
               "\xd4\xe9\xed\xe5      \xc3\xef\xed\xf0\xec\xae\n");
    // gi.cprintf(ent, PRINT_HIGH, "--- ------    ----------         ---- ----
    // ------\n");
    ...
    if (localfound) {
        ...
        // if (maplist.times[level.mapnum][0].time ==
        // sorted_remote_map_best_times[0].time) // wr time
        if (level_items.stored_item_times[0].time ==
            sorted_remote_map_best_times[0].time) {
            // gi.cprintf(ent, PRINT_HIGH, "<%1d> %-6s(%s)%-18s %8s    %-9.3f
            // %d\n", rows_printed, gset_vars->global_localhost_name,
            // HighAscii(wr_txt), this_name,
            // maplist.times[level.mapnum][0].date,
            // maplist.times[level.mapnum][0].time,
            // maplist.times[level.mapnum][0].completions);
            gi.cprintf(ent, PRINT_HIGH,
                       "<%1d> %-6s(%s)%-18s %8s    %-9.3f %d\n", rows_printed,
                       gset_vars->global_localhost_name, HighAscii(wr_txt),
                       this_name, level_items.stored_item_times[0].date,
                       level_items.stored_item_times[0].time,
                       tourney_record[0].completions);
        } else
            // gi.cprintf(ent, PRINT_HIGH, "<%1d> %-9s %-18s %8s    %-9.3f
            // %d\n", rows_printed, gset_vars->global_localhost_name, this_name,
            // maplist.times[level.mapnum][0].date,
            // maplist.times[level.mapnum][0].time,
            // maplist.times[level.mapnum][0].completions);
    ...
    for (i = 0; i < MAX_REMOTE_HOSTS; i++) {
        if (remote_map_best_times[i][0].time > 0) {
            rows_printed++;
            if (remote_map_best_times[i][0].time ==
                sorted_remote_map_best_times[0].time)  // wr time
            {
                gi.cprintf(ent, PRINT_HIGH,
                           "<%1d> %-6s(%s)%-18s %8s    %-9.3f %d\n",
                           rows_printed, pGlobalHostName[i], HighAscii(wr_txt),
                           remote_map_best_times[i][0].name,
                           remote_map_best_times[i][0].date,
                           remote_map_best_times[i][0].time,
                           remote_map_best_times[i][0].completions);
            } else
                gi.cprintf(ent, PRINT_HIGH,
                           "<%1d> %-9s %-18s %8s    %-9.3f %d\n", rows_printed,
                           pGlobalHostName[i], remote_map_best_times[i][0].name,
                           remote_map_best_times[i][0].date,
                           remote_map_best_times[i][0].time,
                           remote_map_best_times[i][0].completions);
...
void Cmd_Remote_Replay(edict_t *ent, int num) {
    ...
    if (num > 0 && num <= MAX_HIGHSCORES) {
        if (level_items.recorded_time_frames[MAX_HIGHSCORES + num] &&
            num <= gset_vars->global_replay_max) {
            // replay found
            ...
            if (sorted_remote_map_best_times[num - 1].time > 0) {
                gi.cprintf(
                    ent, PRINT_HIGH,
                    "Replaying %s who finished in %1.3f seconds (@%s on %8s)\n",
                    sorted_remote_map_best_times[num - 1].name,
                    sorted_remote_map_best_times[num - 1].time,
                    sorted_remote_map_best_times[num - 1].server,
                    sorted_remote_map_best_times[num - 1].date);
            } else
                gi.cprintf(ent, PRINT_HIGH,
                           "Replaying global time %d from %s\n", num,
                           sorted_remote_map_best_times[num - 1].server);
        } else {
            gi.cprintf(ent, PRINT_HIGH,
                       "No global demo available for that position.\n");
            return;
        }
        ...
        if (level_items.recorded_time_frames[MAX_HIGHSCORES + 1]) {
            ...
            if (sorted_remote_map_best_times[0].time > 0) {
                gi.cprintf(
                    ent, PRINT_HIGH,
                    "Replaying %s who finished in %1.3f seconds (@%s on %8s)\n",
                    sorted_remote_map_best_times[0].name,
                    sorted_remote_map_best_times[0].time,
                    sorted_remote_map_best_times[0].server,
                    sorted_remote_map_best_times[0].date);
            } else
                gi.cprintf(ent, PRINT_HIGH,
                           "Replaying global time %d from %s\n", 1,
                           sorted_remote_map_best_times[0].server);
...
void Display_Global_Scoreboard() {
    ...
    *string = 0;
    ...
    sprintf(string + strlen(string),
            "xv 0 yv -16 string2 \"------------  Global Scoreboard  "
            "------------\" ");
    // fancy white dashes commented out, uses more precious bytes and r1q2
    // doesn't like it! sprintf(string + strlen(string), "xv 0 yv -16 string2 \"
    // Global Scoreboard              \" "); sprintf(string + strlen(string),
    // "xv 0 yv -16 string \"------------                     ------------\" ");
    // sprintf(string + strlen(string), "xv 0 yv 0 string2 \"No  Player Time
    // Date     Server\" ");
    sprintf(string + strlen(string),
            "yv 0 string2 \"No  Player           Time     Date     Server\" ");

    for (i = 0; i < MAX_HIGHSCORES; i++) {
        if (sorted_remote_map_best_times[i].time < 0.0001 ||
            strlen(sorted_remote_map_best_times[i].name) < 1)
            continue;
        ...
        if (sorted_remote_map_best_times[i].time > 0.0001) {
            ...
            sprintf(string + strlen(string),
                    "yv %d %s \"%2d%s %-17s%-9.3f%-8s %-s\" ",
                    (rows_printed) * 10 + 16, colorstring, rows_printed + 1,
                    (show_replay == 0 ? " " : chr), this_name,
                    sorted_remote_map_best_times[i].time,
                    sorted_remote_map_best_times[i].date,
                    sorted_remote_map_best_times[i].server);
            rows_printed++;
            show_replay = 0;
        }
        if (rows_printed == MAX_HIGHSCORES) break;
    ...
    if (rows_printed < MAX_HIGHSCORES) {
        for (i = 0; i < (MAX_HIGHSCORES - rows_printed); i++) {
            sprintf(string + strlen(string), "yv %d string \"%2d\" ",
                    (blanks) * 10 + 16, blanks + 1);
            blanks++;
        }
    }
    sprintf(string + strlen(string),
            "xv 40 yv %d string \"replay|race|maptimes (g)lobal <#>\" ",
            blanks * 10 + 24);
...
void Display_Dual_Scoreboards() {
    ...
    *gblstring = 0;
    ...
    sprintf(
        ...
        "xv -230 yv -16 string2 \"----------  Local Scoreboard  ----------\" ");
    sprintf(string + strlen(string),
            "yv 0 string2 \"No   Player          Time      Date \" ");
    for (i = 0; i < MAX_HIGHSCORES; i++) {
        sprintf(colorstring, "string");
    ...
    // Global Board
    sprintf(gblstring + strlen(gblstring),
            "xv 170 yv -16 string2 \"------------  Global Scoreboard  "
            "------------\" ");
    sprintf(gblstring + strlen(gblstring),
            "yv 0 string2 \"No  Player           Time     Date     Server\" ");
    ...
    for (gbli = 0; gbli < MAX_HIGHSCORES; gbli++) {
        if (sorted_remote_map_best_times[gbli].time < 0.0001 ||
            strlen(sorted_remote_map_best_times[gbli].name) < 1)
            continue;

        sprintf(gblcolorstring, "string");
        if (sorted_remote_map_best_times[gbli].time > 0.0001) {
            Com_sprintf(gblthis_name, sizeof(gblthis_name),
                        sorted_remote_map_best_times[gbli].name);
            Highlight_Name(gblthis_name);

            if (gbli < gset_vars->global_replay_max) {
                if (level_items
                        .recorded_time_frames[MAX_HIGHSCORES + (gbli + 1)])
                    gblshow_replay = 1;
            }
            sprintf(gblstring + strlen(gblstring),
                    "yv %d %s \"%2d%s %-17s%-9.3f%-8s %-s\" ",
                    (gblrows_printed) * 10 + 16, gblcolorstring,
                    gblrows_printed + 1, (gblshow_replay == 0 ? " " : gblchr),
                    gblthis_name, sorted_remote_map_best_times[gbli].time,
                    sorted_remote_map_best_times[gbli].date,
                    sorted_remote_map_best_times[gbli].server);
            gblrows_printed++;
            gblshow_replay = 0;
        }
        if (gblrows_printed == MAX_HIGHSCORES) break;
    }
    ...
    if (gblrows_printed < MAX_HIGHSCORES) {
        for (gbli = 0; gbli < (MAX_HIGHSCORES - gblrows_printed); gbli++) {
            sprintf(gblstring + strlen(gblstring), "yv %d string \"%2d\" ",
                    (gblblanks) * 10 + 16, gblblanks + 1);
            gblblanks++;
        }
    }

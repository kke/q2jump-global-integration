...
void Use_Jet(edict_t *ent, gitem_t *item) {
    ...
    gi.sound(ent, CHAN_ITEM, gi.soundindex("items/protect.wav"), 0.3, ATTN_NORM,
             0);
...
gitem_t *FindItem(char *pickup_name) {
    ...
    for (i = 0; i < game.num_items; i++, it++) {
        ...
        if (!Q_stricmp(it->pickup_name, pickup_name)) return it;
    }
...
qboolean Pickup_Powerup(edict_t *ent, edict_t *other) {
    ...
    if ((Q_stricmp(ent->item->pickup_name, "Jetpack") == 0) &&
        (other->client->resp.admin >= aset_vars->ADMIN_GIVE_LEVEL) &&
        (other->client->resp.ctf_team == CTF_TEAM1)) {
    } else if ((other->client->resp.ctf_team == CTF_TEAM1))
        return false;
    ...
    if (Q_stricmp(ent->item->pickup_name, "Jetpack") == 0) {
        ...
        /*otherwise update the burn out time if jetpack is activated*/
            ...
            other->client->Jet_framenum =
                level.framenum + other->client->Jet_remaining;
    }
    if (deathmatch->value) {
...
qboolean Pickup_Adrenaline(edict_t *ent, edict_t *other) {
    if (gametype->value == GAME_CTF) return false;
    if ((other->client->resp.ctf_team == CTF_TEAM1)) return false;
    if (!deathmatch->value) other->max_health += 1;
...
qboolean Pickup_Key(edict_t *ent, edict_t *other) {
    ...
    if (other->client->resp.finished == 1) return false;
    ...
    if (Q_stricmp(ent->item->pickup_name, "weapon clear") == 0) {
        memset(other->client->pers.inventory, 0,
               sizeof(other->client->pers.inventory));  // reset their inventory
    ...
    // resizable starting line, timer is reset when you pass over it, cp's also
    // removed
    if (Q_stricmp(ent->item->pickup_name, "start line") == 0) {
        ...
        other->client->resp.client_think_begin =
            Sys_Milliseconds();  // ui timer reset and internal timer reset 2
    ...
    if (Q_stricmp(ent->item->pickup_name, "cp clear") == 0) {
        if (other->client->resp.store[0].checkpoints > 0)
            gi.cprintf(other, PRINT_HIGH,
                       "%d checkpoint(s) removed from your inventory.\n",
    ...
    if (Q_stricmp(ent->item->pickup_name, "cp resize 1") == 0 &&
        other->client->resp.store[0].rs1_checkpoint != 1) {
        other->client->resp.store[0].checkpoints =
            other->client->resp.store[0].checkpoints + 1;
        other->client->resp.store[0].rs1_checkpoint = 1;
        CPSoundCheck(other);
        if (other->client->resp.ctf_team == CTF_TEAM1)
            gi.cprintf(other, PRINT_HIGH,
                       "You reached checkpoint %d/%d in %1.1f seconds.\n",
                       other->client->resp.store[0].checkpoints,
                       mset_vars->checkpoint_total,
                       other->client->resp.item_timer);
        else
            gi.cprintf(other, PRINT_HIGH,
                       "You reached checkpoint %d/%d in %1.3f seconds.\n",
                       other->client->resp.store[0].checkpoints,
                       mset_vars->checkpoint_total, my_time_decimal);
    }
    if (Q_stricmp(ent->item->pickup_name, "cp resize 2") == 0 &&
    ...
    if (Q_stricmp(ent->item->pickup_name, "cp resize 3") == 0 &&
    ...
    if (Q_stricmp(ent->item->pickup_name, "cp resize 4") == 0 &&
    ...
    if (Q_stricmp(ent->item->pickup_name, "cp resize 5") == 0 &&
    ...
    if (Q_stricmp(ent->item->pickup_name, "Airstrike Marker") == 0 &&
    ...
    if (Q_stricmp(ent->item->pickup_name, "Blue Key") == 0 &&
    ...
    if (Q_stricmp(ent->item->pickup_name, "Data CD") == 0 &&
    ...
    if (Q_stricmp(ent->item->pickup_name, "Data Spinner") == 0 &&
    ...
    if (Q_stricmp(ent->item->pickup_name, "Security Pass") == 0 &&
    ...
    if (Q_stricmp(ent->item->pickup_name, "Power Cube") == 0 &&
    ...
    if (Q_stricmp(ent->item->pickup_name, "Pyramid Key") == 0 &&
    ...
    if (Q_stricmp(ent->item->pickup_name, "Red Key") == 0 &&
...
qboolean Add_Ammo(edict_t *ent, gitem_t *item, int count) {
    ...
    if (ent->client->pers.inventory[index] == max) return false;
...
int PowerArmorType(edict_t *ent) {
    if (!ent->client) return POWER_ARMOR_NONE;
...
qboolean Pickup_PowerArmor(edict_t *ent, edict_t *other) {
    ...
    if (deathmatch->value) {
        ...
        if (!quantity) ent->item->use(other, ent->item);
    }
...
void Touch_Item(edict_t *ent, edict_t *other, cplane_t *plane,
    ...
    if (!other->client ||
        other->client->resp
            .replaying)  // not a client, or if client is watching a replay.
        return;
    if (other->health < 1) return;   // dead people can't pickup
    if (!ent->item->pickup) return;  // not a grabbable item?
...
void PrecacheItem(gitem_t *it) {
    ...
    if (it->pickup_sound) gi.soundindex(it->pickup_sound);
    if (it->world_model) gi.modelindex(it->world_model);
    if (it->view_model) gi.modelindex(it->view_model);
    ...
    while (*s) {
        ...
        if (!strcmp(data + len - 3, "md2"))
            ...
            gi.soundindex(data);
        if (!strcmp(data + len - 3, "pcx")) gi.imageindex(data);
    }
}
...
*/
    {"weapon_grapple", NULL, Use_Weapon, NULL, CTFWeapon_Grapple,
     ...
     /* icon */ "net",
     /* pickup */ "Grapple", 0, 0, NULL, IT_WEAPON, WEAP_GRAPPLE, NULL, 0,
     /* precache */
     "weapons/grapple/grfire.wav weapons/grapple/grpull.wav "
     "weapons/grapple/grhang.wav weapons/grapple/grreset.wav "
...
*/
     ...
     */
    {// this ent allows a plane or box to serve as the finish line, instead of a
     // weapon -Ace DEPRECIATED - trigger_finish should be used instead now
     "weapon_finish", Pickup_Weapon, Use_Weapon, Drop_Weapon, Weapon_Finish,
     "misc/w_pkup.wav", "models/weapons/g_rail/tris.md2", 0,
     "models/weapons/v_rail/tris.md2",
     /* icon */ "w_railgun",
     /* pickup */ "Railgun", 0, 1, "Slugs", IT_WEAPON | IT_STAY_COOP,
     WEAP_RAILGUN, NULL, 0,
...
*/
     ...
     */
    {"item_quad", Pickup_Quad, Use_Quad, Drop_General, NULL, "items/pkup.wav",
...
*/
     ...
     */
    {"cp_resize_1", Pickup_Key, NULL, Drop_General, NULL, NULL,
     "models/items/keys/red_key/tris.md2", EF_GIB, NULL, "k_redkey",
     "cp resize 1", 2, 0, NULL, IT_STAY_COOP | IT_KEY, 0, NULL, 0,
     /* precache */ ""},

    /*QUAKED cp_resize_2 (.5 .5 .5) ?
     */
    {"cp_resize_2", Pickup_Key, NULL, Drop_General, NULL, NULL,
     "models/items/keys/red_key/tris.md2", EF_GIB, NULL, "k_redkey",
     "cp resize 2", 2, 0, NULL, IT_STAY_COOP | IT_KEY, 0, NULL, 0,
     /* precache */ ""},
    /*QUAKED cp_resize_3 (.5 .5 .5) ?
     */
    {"cp_resize_3", Pickup_Key, NULL, Drop_General, NULL, NULL,
     "models/items/keys/red_key/tris.md2", EF_GIB, NULL, "k_redkey",
     "cp resize 3", 2, 0, NULL, IT_STAY_COOP | IT_KEY, 0, NULL, 0,
     /* precache */ ""},
    /*QUAKED cp_resize_4 (.5 .5 .5) ?
     */
    {"cp_resize_4", Pickup_Key, NULL, Drop_General, NULL, NULL,
     "models/items/keys/red_key/tris.md2", EF_GIB, NULL, "k_redkey",
     "cp resize 4", 2, 0, NULL, IT_STAY_COOP | IT_KEY, 0, NULL, 0,
     /* precache */ ""},
    /*QUAKED cp_resize_5 (.5 .5 .5) ?
     */
    {"cp_resize_5", Pickup_Key, NULL, Drop_General, NULL, NULL,
     "models/items/keys/red_key/tris.md2", EF_GIB, NULL, "k_redkey",
     "cp resize 5", 2, 0, NULL, IT_STAY_COOP | IT_KEY, 0, NULL, 0,
     /* precache */ ""},
     ...
     */
    {"item_flag_team1", CTFPickup_Flag, NULL,
     NULL,  // Should this be null if we don't want players to drop it manually?
     NULL, "ctf/flagtk.wav", "players/male/flag1.md2", EF_FLAG1, NULL,
     /* icon */ "i_ctf1",
     /* pickup */ "Red Flag",
     /* width */ 2, 0, NULL, 0, 0, NULL, 0,
     /* precache */ "ctf/flagcap.wav"},

    /*QUAKED item_flag_team2 (1 0.2 0) (-16 -16 -24) (16 16 32)
     */
    {"item_flag_team2", CTFPickup_Flag, NULL,
     NULL,  // Should this be null if we don't want players to drop it manually?
     NULL, "ctf/flagtk.wav", "players/male/flag2.md2", EF_FLAG2, NULL,
     /* icon */ "i_ctf2",
     /* pickup */ "Blue Flag",
     /* width */ 2, 0, NULL, 0, 0, NULL, 0,
     /* precache */ "ctf/flagcap.wav"},
    ...
    /* Resistance Tech */
    {"item_tech1", CTFPickup_Tech, NULL,
     CTFDrop_Tech,  // Should this be null if we don't want players to drop it
                    // manually?
     NULL, "items/pkup.wav", "models/ctf/resistance/tris.md2", EF_ROTATE, NULL,
     /* icon */ "tech1",
     /* pickup */ "Disruptor Shield",
     /* width */ 2, 0, NULL, IT_TECH, 0, NULL, 0,
     /* precache */ "ctf/tech1.wav"},

    /* Strength Tech */
    {"item_tech2", CTFPickup_Tech, NULL,
     CTFDrop_Tech,  // Should this be null if we don't want players to drop it
                    // manually?
     NULL, "items/pkup.wav", "models/ctf/strength/tris.md2", EF_ROTATE, NULL,
     /* icon */ "tech2",
    ...
    /* Haste Tech */
    {"item_tech3", CTFPickup_Tech, NULL,
     CTFDrop_Tech,  // Should this be null if we don't want players to drop it
                    // manually?
     NULL, "items/pkup.wav", "models/ctf/haste/tris.md2", EF_ROTATE, NULL,
     /* icon */ "tech3",
     /* pickup */ "Time Accel",
     /* width */ 2, 0, NULL, IT_TECH, 0, NULL, 0,
    ...
    /* Regeneration Tech */
    {"item_tech4", CTFPickup_Tech, NULL,
     CTFDrop_Tech,  // Should this be null if we don't want players to drop it
                    // manually?
     NULL, "items/pkup.wav", "models/ctf/regeneration/tris.md2", EF_ROTATE,
     NULL,
     /* icon */ "tech4",
     /* pickup */ "AutoDoc",
     /* width */ 2, 0, NULL, IT_TECH, 0, NULL, 0,
    ...
    /* JetPack */
    {"item_jet", Pickup_Powerup, Use_Jet, NULL, NULL, "items/pkup.wav",
     "models/items/keys/data_cd/tris.md2", EF_ROTATE, NULL,
     /* icon */ "p_quad",
     /* pickup */ "Jetpack",
     /* width */ 2, 60, NULL, 0, 0, NULL, 0,
     /* precache */ "ctf/tech4.wav"},
...
void cpbox_touch(edict_t *self, edict_t *other, cplane_t *plane,
                 csurface_t *surf) {
    int my_time;
    float my_time_decimal;
    gitem_t *item;
    ...
    if (self->count >=
        sizeof(player->client->resp.store[0].cpbox_checkpoint) / sizeof(int)) {
        if (trigger_timer(5))
            gi.dprintf(
                "Your count of %i is higher than the max value of %i, you are "
                "a shit mapper.\n",
                self->count,
                sizeof(player->client->resp.store[0].cpbox_checkpoint) /
                        sizeof(int) -
                    1);
        return;
    }
    ...
    if (self->target) {
        ...
        // ckeck if it should reset timer++
            ...
            memset(
                player->client->pers.inventory, 0,
                sizeof(
                    player->client->pers.inventory));  // reset their inventory
            ...
            Stop_Recording(
                player);  // stop the recording for race line alignment
            Start_Recording(player);  // start another recording for this rep
            player->client->resp.item_timer = 0;  // internal timer reset 1
            player->client->resp.client_think_begin =
                Sys_Milliseconds();  // ui timer reset and internal timer reset
                                     // 2
            ...
            for (i = 1; i <= self->count; i++) {  // should it start at 0?
                ...
                if (player->client->resp.store[0].cpbox_checkpoint[i] == 0) {
                    if (trigger_timer(5)) {
                        gi.cprintf(
                            player, PRINT_HIGH,
                            "You need to pick these checkpoints up in the "
    ...
    if (player->client->resp.store[0].cpbox_checkpoint[self->count] != 1) {
        ...
        // in easy give them the int, in hard give them the float, in replay
        // give them relative
        if (player->client->resp.ctf_team == CTF_TEAM1) {
            sprintf(
                cpstring,
                "reached checkpoint %d/%d in %1.1f seconds. (split: %1.1f)\n",
                player->client->resp.store[0].checkpoints,
                mset_vars->checkpoint_total, player->client->resp.item_timer,
                player->client->resp.item_timer -
                    player->client->pers.cp_split);
            ...
            sprintf(
                cpstring,
                "reached checkpoint %d/%d in %1.3f seconds. (split: %1.3f)\n",
                player->client->resp.store[0].checkpoints,
                mset_vars->checkpoint_total, my_time_decimal,
                my_time_decimal - player->client->pers.cp_split);
            player->client->pers.cp_split = my_time_decimal;
            gi.cprintf(player, PRINT_HIGH, "You %s", cpstring);
        } else if (player->client->resp.ctf_team == CTF_NOTEAM &&
                   player->client->resp.replaying &&
                   !player->client->resp.mute_cprep) {
            gi.cprintf(
                player, PRINT_HIGH,
                "%s reached checkpoint %d/%d in about %1.1f seconds. (split: "
                "%1.1f)\n",
                level_items
                    .stored_item_times[player->client->resp.replaying - 1]
                    .owner,
                player->client->resp.store[0].checkpoints,
                mset_vars->checkpoint_total,
                (player->client->resp.replay_frame / 10) - 0.1,
                ((player->client->resp.replay_frame / 10) - 0.1) -
                    player->client->pers.cp_split);
            player->client->pers.cp_split =
                (player->client->resp.replay_frame / 10) - 0.1;
        ...
        for (i = 0; i < maxclients->value; i++) {
            cl_ent = g_edicts + 1 + i;
            if (!cl_ent->inuse || !cl_ent->client->chase_target) continue;
            if (cl_ent->client->chase_target->client->resp.ctf_team ==
                CTF_NOTEAM)
                continue;
            if (Q_stricmp(cl_ent->client->chase_target->client->pers.netname,
                          player->client->pers.netname) == 0) {
                gi.cprintf(cl_ent, PRINT_HIGH, "%s %s",
                           player->client->pers.netname, cpstring);
                memcpy(cl_ent->client->resp.store[0].cpbox_checkpoint,
                       player->client->resp.store[0].cpbox_checkpoint,
                       sizeof(player->client->resp.store[0].cpbox_checkpoint));
            }
        }
        // play a sound for it
...
void SP_jump_time_think(edict_t *ent) {
    ...
    int h1, h2, h3, h4;
    cur_time =
        ((mset_vars->timelimit * 60) + (map_added_time * 60) - level.time);
...
void cpeffect_think(edict_t *self) {
    ...
    for (i = 0; i < maxclients->value; i++) {
        ...
        if (!temp_ent->inuse || !temp_ent->client) continue;
...
void SP_jump_cpeffect(edict_t *ent) {
    ...
    if (ent->count < 0 || ent->count > 63) {
        gi.dprintf(
            "jump_cpeffect at %s does not have the correct count-value (0-63, "
            "current value: %d\n",
...
void cpwall_think(edict_t *self) {
    ...
    for (i = 0; i < maxclients->value; i++) {
        ...
        if (!temp_ent->inuse || !temp_ent->client) continue;
        if (self->style != 1 &&
            temp_ent->client->resp.store[0].checkpoints >= self->count)
            continue;
...
void cpwall_touch(edict_t *self, edict_t *other, cplane_t *plane,
                  csurface_t *surf) {
    if (!other->client) return;
    if (other->client->resp.ctf_team == CTF_TEAM1 ||
        other->client->resp.ctf_team == CTF_TEAM2) {
        if (self->style != 1 &&
            other->client->resp.store[0].checkpoints < self->count) {
            VectorCopy(other->s.old_origin, other->s.origin);
            VectorClear(other->velocity);
            if (trigger_timer(5)) {
                gi.cprintf(other, PRINT_HIGH,
                           "You need %d checkpoint(s) to pass this barrier.\n",
                           self->count);
            }
            ...
            if (trigger_timer(2))
                gi.cprintf(
                    other, PRINT_HIGH,
                    "You need %d lap checkpoint(s) to pass this barrier.\n",
                    self->count);
        }
    }
}
...
// if spawnflag 1 is set, it works the other way around. Activates when
// checkpoint<=count.
...
//---one-way-wall start
void one_way_wall_touch(edict_t *self, edict_t *other, cplane_t *plane,
                        csurface_t *surf) {
    // not a client...
    if (!other->client) return;
    ...
    VectorCopy(other->velocity, vel);  // I'm fairly sure I have to copy the
                                       // velocity before normalizing it.
    VectorNormalize(vel);
    AngleVectors(self->s.angles, forward, NULL,
                 NULL);  // get angle vector of the wall

    dot = DotProduct(vel, forward);  // dot product less than zero means we're
                                     // going against the wall.

    // setting positive values here will result in the angle to be more strict
    // (for example (dot <= 0.7) should only allow 45 degrees deviations from
    // the specified angle). It's up to you if you want to tweak this. Or maybe
    ...
    if (dot <= 0) {
        ...
        if (trigger_timer(5)) {
            if (!(self->spawnflags & 1)) {
                ...
                gi.cprintf(other, PRINT_HIGH,
                           "You need %.0f speed to pass the wall this way.\n",
                           self->speed);
            }
        }
    }
...
// Allows the player to go through this entity only if his direction matches the
// angle set by mapper. There is an option to make the player able go against
// the direction if he reaches a certain speed.
//
// Settings:
//		- angle/angles - this property specifies the direction player is
// allowed to pass in.
//		- spawnflags - if set to 1 this entity will validate player's
// speed. Once his speed is equal 					   or
// higher he can go through, no matter the angle
//		- speed - set this property so entity can validate it. Works
// only with spawnflags 1.
//
// Notes:
// The angle can be within 90 degrees from the entity's angle and the player
// will still pass through. This can be altered with dot product comparison (if
// you guys want the angle window to be smaller).
//
// IMPORTANT: THE WALL MUST BE THICK - otherwise at high speeds you can pass
// through with 20fps. For 4000ups I think at least 192 units. Maybe more - I'll
// do more tests.
